...
function getPreview(question: Question) {
  return question.question_text || question.answer_text || "ASQ Question";
}

export default function AsqPracticeList({
  initialQuestions,
}: {
    ...
    <section className="mx-auto block w-full max-w-[84%]">
      ...
      <div className="mx-auto w-[97.5%] space-y-1">
        {paginatedQuestions.map((item, index) => (
          <Link
            key={item.id}
            href={`/pte/speaking/asq/${item.id}`}
            onClick={() => {
              saveQuestionOrder("asq", questionIds);
            }}
            className="block"
          >
              ...
              <div className="relative">
                ...
                <div className="relative flex items-start justify-center gap-5 px-5 py-4 sm:px-6">
                  ...
                  <div className="hidden w-[95px] flex-shrink-0 items-center justify-center md:flex">
                    <MasteryProgress correct={item.best_score} total={10} />
                  </div>
                </div>
              </div>
            </article>
