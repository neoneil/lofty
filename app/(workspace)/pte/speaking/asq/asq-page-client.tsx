...
type Props = {
  questions: ComponentProps<typeof AsqPracticeList>["initialQuestions"];
...
export default function AsqPageClient({ questions, questionInfo, filters, pagination }: Props) {
  ...
  return (
    <section className={`w-full space-y-2 transition-opacity ${questionBank.isPending ? "opacity-70" : ""}`}>
      ...
      <AsqPracticeList initialQuestions={questions} pagination={pagination} onPageChange={questionBank.goToPage} />
    </section>
  );
}
