...
export default async function SgdQuestionDetailPage({ params }: PageProps) {
  const { id } = await params;
  const { supabase } = await requireUser(`/pte/speaking/sgd/${id}`);
    ...
    <div className="mt-1">
      <section className="space-y-6">
        <section className="rounded border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm sm:p-7">
          ...
          <SgdDetailClient question={question} />
        </section>
      </section>
    </div>
  );
