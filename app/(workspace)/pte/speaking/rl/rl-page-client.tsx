...
type Props = {
  questions: ComponentProps<typeof RlPracticeList>["initialQuestions"];
...
export default function RlPageClient({ questions, questionInfo, filters, pagination }: Props) {
  ...
  return (
    <section className={`w-full space-y-2 transition-opacity ${questionBank.isPending ? "opacity-70" : ""}`}>
      ...
      <RlPracticeList initialQuestions={questions} pagination={pagination} onPageChange={questionBank.goToPage} />
    </section>
  );
}
