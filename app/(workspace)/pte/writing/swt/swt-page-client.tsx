...
type Props = {
  questions: ComponentProps<typeof SwtList>["initialQuestions"];
...
export default function SwtPageClient({ questions, questionInfo, filters, pagination }: Props) {
  ...
  return (
    <section className={`w-full space-y-2 transition-opacity ${questionBank.isPending ? "opacity-70" : ""}`}>
      ...
      <SwtList initialQuestions={questions} pagination={pagination} onPageChange={questionBank.goToPage} />
    </section>
  );
}
