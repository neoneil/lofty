...
type Props = {
  questions: ComponentProps<typeof EssayList>["initialQuestions"];
...
export default function EssayPageClient({ questions, questionInfo, filters, pagination }: Props) {
  ...
  return (
    <section className={`w-full space-y-2 transition-opacity ${questionBank.isPending ? "opacity-70" : ""}`}>
      ...
      <EssayList initialQuestions={questions} pagination={pagination} onPageChange={questionBank.goToPage} />
    </section>
  );
}
