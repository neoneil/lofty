...
type Props = {
  questions: ComponentProps<typeof SstList>["initialQuestions"];
...
export default function SstPageClient({ questions, questionInfo, filters, pagination }: Props) {
  ...
  return (
    <section className={`w-full space-y-2 transition-opacity ${questionBank.isPending ? "opacity-70" : ""}`}>
      ...
      <SstList initialQuestions={questions} pagination={pagination} onPageChange={questionBank.goToPage} />
    </section>
  );
}
