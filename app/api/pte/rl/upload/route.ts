...
export async function POST(req: Request) {
  try {
    ...
    const questionExists = await ensurePteQuestionExists(supabase, "rl", questionId);
    if (!questionExists) {
    ...
    if (insertError) {
      console.error("RL recording insert error:", insertError);
    ...
    if (isStudentRecordingUploadError(error)) {
      console.error("RL recording upload error:", error);
