...
export async function POST(req: Request) {
  try {
    ...
    const questionExists = await ensurePteQuestionExists(supabase, "di", questionId);
    if (!questionExists) {
      return NextResponse.json({ error: "question not found" }, { status: 404 });
    }

    const audioStorageKey = await uploadStudentRecordingToPrivateR2({ file, questionSource: "di", userId: user.id });
    ...
    try {
      ...
      console.error("DI stats update error:", statsError);
