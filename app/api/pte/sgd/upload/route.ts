...
export async function POST(req: Request) {
  try {
    ...
    const questionExists = await ensurePteQuestionExists(supabase, "sgd", questionId);
    if (!questionExists) {
      return NextResponse.json({ error: "question not found" }, { status: 404 });
    }

    const audioStorageKey = await uploadStudentRecordingToPrivateR2({ file, questionSource: "sgd", userId: user.id });
    ...
    try {
      ...
      console.error("SGD stats update error:", statsError);
