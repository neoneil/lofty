...
const LISTENING_BOOKS = [21, 20, 19, 18, 17, 16] as const;
    ...
    <main className="container-main space-y-6 py-5 sm:py-7">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {LISTENING_BOOKS.map((bookNumber) => (
          <Link key={bookNumber} href={`/ielts/listening?book=${bookNumber}`} className="group block focus:outline-none">
            <CambridgeListeningBookCover bookNumber={bookNumber} />
          </Link>
        ))}
      </section>
    </main>
...
export function IeltsListeningTestSelector({ bookNumber, data }: { bookNumber: number; data: IeltsBookPracticeData }) {
  ...
  return (
    <main className="container-main space-y-5 py-5 sm:py-7">
      ...
      <section className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--card)] p-5 shadow-[var(--shadow-md)] sm:p-7">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-[var(--text)] sm:text-4xl">选择听力 Test</h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--text-soft)] sm:text-base">每套 Test 分为 4 个 Part，每个 Part 10 道题。进入后左侧答题，右侧字幕区默认收起。</p>
          </div>
          <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-soft)] p-4 text-sm text-[var(--text-soft)]">
            <div className="mb-1 flex items-center gap-2 font-semibold text-[var(--text)]"><Timer size={17} className="text-[var(--primary)]" />Listening Timer</div>
            <div>40 questions · 4 parts</div>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {tests.map((test) => (
          <IeltsTestEntryCard key={test.id} moduleType="listening" bookNumber={bookNumber} testNumber={test.test_number} title={test.title || `Cambridge IELTS ${bookNumber} Listening Test ${test.test_number}`} href={`/ielts/listening?book=${bookNumber}&test=${test.test_number}`} />
        ))}
      </section>
    </main>
  );
...
function CambridgeListeningBookCover({ bookNumber }: { bookNumber: number }) {
        ...
        <div className="flex gap-4">
          <div className={cn("relative aspect-[0.74] w-28 shrink-0 overflow-hidden rounded-[var(--radius-md)] border shadow-[var(--shadow-md)] sm:w-32", isRed ? "border-red-900/20 bg-[#e9153d]" : "border-blue-950/30 bg-[#123c67]")}>
            ...
            <div className="relative flex h-full flex-col justify-between p-4 text-white">
              ...
              <div className="max-w-[72px] text-[9px] font-semibold uppercase leading-3 tracking-[0.1em] opacity-90">Cambridge Listening Test</div>
            </div>
          </div>
          <div className="flex min-w-0 flex-1 flex-col justify-between py-1">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--text-soft)]">进入后选择 Test 1-4，开始听力机考练习。</p>
            </div>
