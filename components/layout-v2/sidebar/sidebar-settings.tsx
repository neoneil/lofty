...
import { Palette, RotateCcw, Settings, SunMoon, X } from "lucide-react";

import { applyUiSkin, UiSkinPicker } from "@/components/settings/ui-skin-picker";
...
export function SidebarSettings({ collapsed }: SidebarSettingsProps) {
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-label="打开设置" title={collapsed ? "设置" : undefined} className={cn("group flex h-12 w-full cursor-pointer items-center rounded-[var(--radius-xsm)] text-[var(--text-soft)] transition-all duration-300 hover:bg-[var(--bg-soft)] hover:text-[var(--text)]", collapsed ? "justify-center px-0" : "justify-between px-2.5 sm:px-3")}>
      ...
      {open ? createPortal(<div role="dialog" aria-modal="true" aria-labelledby="sidebar-settings-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }} className="fixed inset-0 z-[120] flex items-end justify-center overflow-y-auto bg-black/55 p-0 backdrop-blur-sm sm:items-center sm:p-4"><Card className="my-0 max-h-[92dvh] w-full overflow-hidden rounded-b-none rounded-t-[var(--radius-lg)] shadow-[var(--shadow-lg)] sm:my-auto sm:max-w-3xl sm:rounded-[var(--radius-lg)]"><CardContent className="p-0"><div className="flex flex-col gap-4 border-b border-[var(--border)] p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6"><div><Badge>Preferences</Badge><h2 id="sidebar-settings-title" className="mt-3 text-2xl font-semibold text-[var(--text)]">设置</h2><p className="mt-1 text-sm text-[var(--text-soft)]">管理当前设备的网站外观与使用偏好。</p></div><div className="flex shrink-0 items-center gap-2"><Button type="button" variant="secondary" onClick={() => applyUiSkin("default")} className="gap-2"><RotateCcw size={15} />恢复默认</Button><Button type="button" onClick={() => setOpen(false)}>完成</Button><button type="button" onClick={() => setOpen(false)} aria-label="关闭设置" className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-[var(--radius-md)] text-[var(--text-faint)] transition-colors hover:bg-[var(--bg-soft)] hover:text-[var(--text)]"><X size={18} /></button></div></div>
