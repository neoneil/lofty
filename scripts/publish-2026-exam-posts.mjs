...
const manifest = JSON.parse(
  readFileSync(path.join(root, "content/posts/2026-exam-prep-posts.json"), "utf8"),
);
const prefix = "images/posts/2026-exam-prep";
const region = "auto";
const service = "s3";
const publicBase = (
...
async function uploadCover(post) {
  ...
  const filePath = path.join(
    root,
    "output/imagegen/2026-exam-posts",
    `${post.slug}.webp`,
  );
...
async function main() {
  ...
  writeFileSync(
    path.join(root, "tmp/posts/2026-exam-posts-before.json"),
  ...
  writeFileSync(
    path.join(root, "tmp/posts/2026-exam-posts-published.json"),
