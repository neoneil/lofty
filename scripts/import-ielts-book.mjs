...
async function main() {
  const targetBookNumber = Number(process.argv[2] ?? 21);
  if (![21, 20, 19, 18, 17, 16].includes(targetBookNumber)) {
    throw new Error(`Unsupported IELTS book number: ${process.argv[2]}`);
  }
  ...
  const book = home.bookList.find((item) => item.id === targetBookNumber);
  if (!book) throw new Error(`Cambridge IELTS ${targetBookNumber} not found in source API`);
  const papers = book.paperList.slice().sort((a, b) => Number(a.paperNumber.replace(/\D/g, "")) - Number(b.paperNumber.replace(/\D/g, "")));

  const [bookRow] = await select("cambridge_books", `?book_number=eq.${targetBookNumber}&select=id,book_number,title`);
  if (!bookRow) throw new Error(`ielts.cambridge_books book_number=${targetBookNumber} not found`);
  ...
  if (moduleIds.length) {
    ...
    console.log(`Cleaned existing IELTS ${targetBookNumber} sections: ${sectionIds.length}`);
  }
  await remove("assets", `?storage_path=like.${targetBookNumber}/*`);
  ...
  for (const paper of papers) {
    ...
    for (const classify of [1, 2, 3]) {
      ...
      console.log(`Importing IELTS ${targetBookNumber} Test ${testNumber} ${moduleType}...`);
      const testModule = modulesByTestAndType.get(`${test.id}:${moduleType}`);
      if (!testModule) throw new Error(`Missing local module row for Test ${testNumber} ${moduleType}`);
      ...
      const basePath = `${targetBookNumber}/test-${testNumber}/${moduleType}`;
      const assetRows = [];
