---
book_id: "1249e423-5c10-498d-ac08-2e90df646c5f"
book_number: 21
book_title: "Cambridge IELTS 21"
book_is_active: true
test_id: "92705880-e1f1-4d09-89b2-45652415323f"
test_number: 1
test_title: "Cambridge IELTS 21 Test 1"
module_id: "a1736f94-930d-458e-beb8-c4e4d5d1c496"
module_type: "reading"
module_title: "Reading"
duration_minutes: 60
module_sort_order: 2
section_id: "67ea5018-808f-4a0b-85a9-80da99c6beba"
section_number: 2
section_title: "Passage 2"
section_instruction: "<p>Read and answer Questions 14–26.</p>"
passage_title: null
section_sort_order: 2
module_raw_data_json: |
  {}
section_raw_data_json: |
  {
    "id": "476",
    "desc": "<p>Read and answer Questions 14–26.</p>",
    "audio": "",
    "pages": [
      {
        "desc": "<p>Reading Passage 2 has seven sections, <strong>A–G</strong>.</p><p>Which section contains the following information?</p><p><i>Write the correct letter, <strong>A–G</strong>, in boxes 14–17 on your answer sheet.</i></p>",
        "type": "28",
        "title": "Questions14 - 17",
        "content": "<br><strong class=\"mock-question-number\">14</strong> examples of strategies to decrease the noise that the public are exposed to&nbsp;#####-14-##### \n<br> \n<strong class=\"mock-question-number\">15</strong> data indicating the extent of the problem of excessive noise&nbsp;#####-15-##### \n<br> \n<strong class=\"mock-question-number\">16</strong> a description of physiological changes in our bodies when we hear sudden noises&nbsp;#####-16-##### \n<br> \n<strong class=\"mock-question-number\">17</strong> evidence that a relatively quiet environment can be more beneficial than a totally&nbsp;silent one&nbsp;#####-17-#####",
        "realType": "7",
        "sections": [
          {
            "id": "1493",
            "res": "",
            "desc": "",
            "type": "28",
            "title": "14 examples of strategies to decrease the noise that the public are exposed to",
            "partId": 476,
            "options": [
              {
                "id": 6772,
                "title": "A",
                "legend": 0,
                "sortOrder": 0
              },
              {
                "id": 6773,
                "title": "B",
                "legend": 0,
                "sortOrder": 1
              },
              {
                "id": 6774,
                "title": "C",
                "legend": 0,
                "sortOrder": 2
              },
              {
                "id": 6775,
                "title": "D",
                "legend": 0,
                "sortOrder": 3
              },
              {
                "id": 6776,
                "title": "E",
                "legend": 0,
                "sortOrder": 4
              },
              {
                "id": 6777,
                "title": "F",
                "legend": 0,
                "sortOrder": 5
              },
              {
                "id": 6778,
                "title": "G",
                "legend": 0,
                "sortOrder": 6
              }
            ],
            "paperId": 53,
            "classify": "2",
            "realType": "7",
            "chapterId": "1237",
            "questions": [
              {
                "id": "13090",
                "desc": "",
                "sort": "14",
                "title": "14 examples of strategies to decrease the noise that the public are exposed to",
                "answerId": "",
                "optionId": "",
                "optionIds": "6774",
                "sectionId": "1493",
                "questionNo": 14,
                "answerValue": "",
                "answerExplain": "题干关键词strategies to decrease noise the public are exposed to（降低公众噪音暴露的策略）。C段首句说several cities have been working to turn the volume down（调低音量）。整段列举quiet buses、降噪路面、leaf blower禁令等措施。选C。\n题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。\n题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。\n题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。",
                "firstQuestion": false
              },
              {
                "id": "13091",
                "desc": "",
                "sort": "15",
                "title": "15 data indicating the extent of the problem of excessive noise",
                "answerId": "",
                "optionId": "",
                "optionIds": "6773",
                "sectionId": "1493",
                "questionNo": 15,
                "answerValue": "",
                "answerExplain": "题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。",
                "firstQuestion": false
              },
              {
                "id": "13092",
                "desc": "",
                "sort": "16",
                "title": "16 a description of physiological changes in our bodies when we hear sudden noises",
                "answerId": "",
                "optionId": "",
                "optionIds": "6772",
                "sectionId": "1493",
                "questionNo": 16,
                "answerValue": "",
                "answerExplain": "题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。",
                "firstQuestion": false
              },
              {
                "id": "13093",
                "desc": "",
                "sort": "17",
                "title": "17 evidence that a relatively quiet environment can be more beneficial than a totally silent one",
                "answerId": "",
                "optionId": "",
                "optionIds": "6778",
                "sectionId": "1493",
                "questionNo": 17,
                "answerValue": "",
                "answerExplain": "题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。",
                "firstQuestion": false
              }
            ]
          }
        ],
        "questionGroupId": 898
      },
      {
        "desc": "<p>Complete the summary below.<br>Choose <strong>ONE WORD ONLY</strong> from the passage for each answer.<br>Write your answers in boxes 18–21 on your answer sheet.</p>",
        "type": "10",
        "title": "Questions 18–21",
        "content": "<div style=\"text-align:center;\"><br><span style=\"font-size:22px;\"><strong>Flotation Tanks</strong>&nbsp;</span><br>&nbsp;</div><p>According to Justin Feinstein, flotation tanks allow people to concentrate on their own&nbsp;<span data-question-id=\"18\">#####-18-#####</span>&nbsp; , which helps them relax and enables them to meditate.&nbsp;<br><br>Feinstein and his colleagues conducted an experiment in which 50 people, who were&nbsp;all suffering from stress and related issues, were given a&nbsp;<span data-question-id=\"19\">#####-19-#####</span>&nbsp;to&nbsp;complete before and after using a flotation tank. Participants reported a reduction&nbsp;in their symptoms after an hour in the tank, together with signs of relaxation and&nbsp;improved general&nbsp;<span data-question-id=\"20\">#####-20-#####</span>&nbsp;.&nbsp;<br><br>In another experiment, the researchers had 48 people spend periods of 90 minutes&nbsp;either lying back in a chair or floating in a tank. Brain scans then revealed that those&nbsp;people who had been in a tank had decreased activity in parts of the brain associated with&nbsp;<span data-question-id=\"21\">#####-21-#####</span>&nbsp;.</p>",
        "realType": "11",
        "sections": [
          {
            "id": "1494",
            "res": "",
            "desc": "",
            "type": "10",
            "title": "18-21.",
            "partId": 476,
            "options": [
              {
                "id": 6779,
                "title": "breath",
                "legend": 0,
                "sortOrder": 0
              },
              {
                "id": 6780,
                "title": "questionnaire",
                "legend": 0,
                "sortOrder": 1
              },
              {
                "id": 6781,
                "title": "wellbeing",
                "legend": 0,
                "sortOrder": 2
              },
              {
                "id": 6782,
                "title": "depression",
                "legend": 0,
                "sortOrder": 3
              }
            ],
            "paperId": 53,
            "classify": "2",
            "realType": "11",
            "chapterId": "1238",
            "questions": [
              {
                "id": "13094",
                "desc": "",
                "sort": "18",
                "title": "18.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6779",
                "sectionId": "1494",
                "questionNo": 18,
                "answerValue": "",
                "answerExplain": "题干说concentrate on（专注于）什么帮助放松和冥想。D段说focus on the breath（专注于呼吸）helps people reach a relaxed or meditative state（达到放松或冥想状态）。focus on = concentrate on。填breath。\n题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。\n题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。\n题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。",
                "firstQuestion": false
              },
              {
                "id": "13095",
                "desc": "",
                "sort": "19",
                "title": "19.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6780",
                "sectionId": "1494",
                "questionNo": 19,
                "answerValue": "",
                "answerExplain": "题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。",
                "firstQuestion": false
              },
              {
                "id": "13096",
                "desc": "",
                "sort": "20",
                "title": "20.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6781",
                "sectionId": "1494",
                "questionNo": 20,
                "answerValue": "",
                "answerExplain": "题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。",
                "firstQuestion": false
              },
              {
                "id": "13097",
                "desc": "",
                "sort": "21",
                "title": "21.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6782",
                "sectionId": "1494",
                "questionNo": 21,
                "answerValue": "",
                "answerExplain": "题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。",
                "firstQuestion": false
              }
            ]
          }
        ],
        "questionGroupId": 899
      },
      {
        "desc": "<p><i>Look at the following statements (Questions 22–26) and the list of people below.</i><br><i>Match each statement with the correct person, <strong>A</strong>, <strong>B</strong>, <strong>C</strong> or <strong>D</strong>.</i><br><i>Write the correct letter, <strong>A</strong>, <strong>B</strong>, <strong>C</strong> or <strong>D</strong>, in boxes 22–26 on your answer sheet.</i><br><i><strong>NB</strong> You may use any letter more than once.</i></p>",
        "type": "22",
        "title": "Questions 22–26",
        "content": "<p><strong>List of people</strong></p>",
        "realType": "6",
        "sections": [
          {
            "id": "1495",
            "res": "",
            "desc": "List of People",
            "type": "22",
            "title": "22 It is unpleasant and upsetting for people to be placed in a silent environment against their will.",
            "partId": 476,
            "options": [
              {
                "id": 6783,
                "title": "A. Nick Antonio",
                "legend": 0,
                "sortOrder": 0
              },
              {
                "id": 6784,
                "title": "B. Justin Feinstein",
                "legend": 0,
                "sortOrder": 1
              },
              {
                "id": 6785,
                "title": "C. Tal Dotan Ben-Soussan",
                "legend": 0,
                "sortOrder": 2
              },
              {
                "id": 6786,
                "title": "D. Eric Pfeifer",
                "legend": 0,
                "sortOrder": 3
              }
            ],
            "paperId": 53,
            "classify": "2",
            "realType": "6",
            "chapterId": "1239",
            "questions": [
              {
                "id": "13098",
                "desc": "",
                "sort": "22",
                "title": "22 It is unpleasant and upsetting for people to be placed in a silent environment against their will.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6785",
                "sectionId": "1495",
                "questionNo": 22,
                "answerValue": "",
                "answerExplain": "题干说违背意愿被置于安静环境中让人unpleasant and upsetting（不愉快和痛苦）。F段Ben-Soussan说When people do not want silence, it can be very distressing（不想要的安静令人痛苦）。完全对应。选C。\n题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。\n题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。\n题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。\n题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）",
                "firstQuestion": false
              },
              {
                "id": "13099",
                "desc": "",
                "sort": "23",
                "title": "23 The trend towards creating quieter urban locations is likely to increase in the coming years.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6783",
                "sectionId": "1495",
                "questionNo": 23,
                "answerValue": "",
                "answerExplain": "题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。",
                "firstQuestion": false
              },
              {
                "id": "13100",
                "desc": "",
                "sort": "24",
                "title": "24 When our body's senses are completely deprived of input, our minds compensate for this by creating the illusion of images and sounds.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6784",
                "sectionId": "1495",
                "questionNo": 24,
                "answerValue": "",
                "answerExplain": "题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。",
                "firstQuestion": false
              },
              {
                "id": "13101",
                "desc": "",
                "sort": "25",
                "title": "25 Even a short amount of silent time can have a positive impact.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6786",
                "sectionId": "1495",
                "questionNo": 25,
                "answerValue": "",
                "answerExplain": "题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。",
                "firstQuestion": false
              },
              {
                "id": "13102",
                "desc": "",
                "sort": "26",
                "title": "26 External and internal quietness makes us more conscious of events occurring in our surroundings and helps us react appropriately to these events.",
                "answerId": "",
                "optionId": "",
                "optionIds": "6785",
                "sectionId": "1495",
                "questionNo": 26,
                "answerValue": "",
                "answerExplain": "题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）",
                "firstQuestion": false
              }
            ]
          }
        ],
        "questionGroupId": 900
      }
    ],
    "title": "Passage 2",
    "value": "",
    "mobile": "",
    "content": "<div style=\"text-align:center;\"><span style=\"font-size:18px;\"><strong>Why we need silence</strong></span></div><p><br><strong>A</strong> Humans are finely attuned to noise, and for good reason. From an evolutionary&nbsp;perspective, sounds give us vital information, helping us navigate the world and avoid danger. To help ensure loud or unexpected noises get the attention they deserve, our internal chemistry alters in response to them. Our blood pressure goes up, muscles&nbsp;tense and glands release hormones that prepare us for fight or flight. In the short term, this is a good thing. When we are exposed to too much noise over the long term, however, those responses can lead to a multitude of health issues, from sleep disturbance to even cardiovascular disease.&nbsp;<br><br><strong>B</strong> The World Health Organization has recently designated excessive noise as an 'underestimated threat' to public health, and has said that people living in cities such as Mumbai, Tokyo and Buenos Aires are being exposed to far more than the recommended 40 decibels of noise at night. A report from the European Environment Agency concluded that noise was an ongoing and widespread issue in Europe, with at least 1 in 5 people consistently exposed to levels considered harmful to health. 'There are no \"earlids\" that can protect your brain from noise,' says Nick Antonio, an acoustic consultant who has contributed to the British and international standards for noise.&nbsp;<br><br><strong>C</strong> The good news is that several cities have been working to turn the volume down. One of the first to do so was London. 'By providing recommendations for quieter buses, reducing noise from roads and also controlling noise from aircraft, they were able to make the city quieter,' says Antonio. Other cities have introduced noise-reducing road coatings, for instance, alongside greenery that muffles sound. Some solutions are more specific: Washington DC's ban on petrol-powered leaf blowers came into effect recently, while in New York City, legislation has been approved to fine people who modify their vehicles to make them noisier. 'People are seeing the benefits of these more quiet environments in their cities,' Antonio says. 'I expect we will see much more of this in the future.'&nbsp;<br><br><strong>D</strong> Researchers are also seeking to understand what aspects of silent experiences are most beneficial to our health. One of the best-researched is the flotation tank: a lightproof, soundproof tank of salt water in which a person floats as a form of deep relaxation. While some people experience altered perception in the tanks, involving subtle humming sounds and visual effects, these effects are benign and do not detract from the benefits of the experience, says Justin Feinstein, a clinical neuropsychologist. 'When you don't have external sensory stimuli coming in, the brain tries to fill the void to make sense of this dark and silent world,' he explains. 'In these tanks, some people can even hear the sound of their eyes blinking,' says Feinstein. 'But it is the ability to focus on the breath that helps people reach a relaxed or meditative state.'&nbsp;<br><br><strong>E </strong>To further explore flotation tanks as a therapeutic tool, Feinstein and his colleagues recruited 50 people with a variety of conditions related to stress and had them answer a questionnaire prior to and following a flotation session. Participants reported decreases in muscle tension, pain and symptoms of their conditions after a single, 1-hour float, alongside an increase in feelings of relaxation and overall wellbeing.&nbsp;<br><br>Less is known about what effects sensory deprivation can have on the brain. To investigate, Feinstein's team had 48 people participate in either three 90-minute float sessions or three 90-minute periods of relaxing on a chair which reclined. Participants had their brains scanned using functional magnetic resonance imaging at the beginning and end of the trial. Float sessions uniquely decreased activity in the default mode network (DMN), a collection of brain regions commonly linked with depression. Feinstein says it is an exciting finding, because flotation tanks seem to offer a way of 'resetting' our nervous system to prevent it from getting out of balance.&nbsp;<br><br><strong>F </strong>Neurobiologist Tal Dotan Ben-Soussan is also an advocate of silence as a therapy. 'When we find ways to be quiet, we are not only quiet in our environment, but quiet in our inner selves,' she explains. 'This allows us to be more aware of what is happening around us and what the situation may require from us so we can provide [a more] adequate response.'&nbsp;<br><br>Not everyone will benefit from silence to the same extent, but Ben-Soussan says one characteristic is key: the person must need to want to engage in the experience. 'We see from animal models and human studies that volition and intentionality is important,' she says. 'When people do not want silence, it can be very distressing.'&nbsp;<br><br><strong>G&nbsp;</strong>Eric Pfeifer, a psychotherapy researcher, also concedes that some people may not benefit from silence, particularly those who are in a heightened state of stress. 'People in these states may not be able to relax or calm down in a silent condition,' he says. Professional guidance can be useful, he adds, allowing people to approach silence slowly so that they can gradually enjoy the benefits. And Pfeifer is convinced that silence is more attainable in everyday life than people think. First, complete silence isn't necessary. In a recent study, he found that participants reported more relaxation and less boredom when they sat quietly in an outdoor garden compared with a completely silent room. Second, Pfeifer believes we don't need a lot of silence to gain benefits. 'You don't need to spend hours in silence,' he says. 'It is likely better to have more frequency of silence for a few minutes at a time than a longer period of silence only once a week. Just finding those places in your daily life where you can find some silence … can make a big difference.'</p>",
    "paperId": "53",
    "classify": "2",
    "isSubmit": "1",
    "subjectId": 159,
    "writingList": [],
    "originalList": [],
    "analysisAudio": ""
  }
questions_json: |
  [
    {
      "id": "2961d822-6527-4849-b40c-161aa09bb111",
      "section_id": "67ea5018-808f-4a0b-85a9-80da99c6beba",
      "question_number_start": 14,
      "question_number_end": 17,
      "question_type": "7",
      "prompt": "Questions14 - 17",
      "instruction": "<p>Reading Passage 2 has seven sections, <strong>A–G</strong>.</p><p>Which section contains the following information?</p><p><i>Write the correct letter, <strong>A–G</strong>, in boxes 14–17 on your answer sheet.</i></p>",
      "content": {
        "page_desc": "<p>Reading Passage 2 has seven sections, <strong>A–G</strong>.</p><p>Which section contains the following information?</p><p><i>Write the correct letter, <strong>A–G</strong>, in boxes 14–17 on your answer sheet.</i></p>",
        "questions": [
          {
            "id": "13090",
            "desc": "",
            "sort": "14",
            "title": "14 examples of strategies to decrease the noise that the public are exposed to",
            "answerId": "",
            "optionId": "",
            "optionIds": "6774",
            "sectionId": "1493",
            "questionNo": 14,
            "answerValue": "",
            "answerExplain": "题干关键词strategies to decrease noise the public are exposed to（降低公众噪音暴露的策略）。C段首句说several cities have been working to turn the volume down（调低音量）。整段列举quiet buses、降噪路面、leaf blower禁令等措施。选C。\n题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。\n题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。\n题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。",
            "firstQuestion": false
          },
          {
            "id": "13091",
            "desc": "",
            "sort": "15",
            "title": "15 data indicating the extent of the problem of excessive noise",
            "answerId": "",
            "optionId": "",
            "optionIds": "6773",
            "sectionId": "1493",
            "questionNo": 15,
            "answerValue": "",
            "answerExplain": "题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。",
            "firstQuestion": false
          },
          {
            "id": "13092",
            "desc": "",
            "sort": "16",
            "title": "16 a description of physiological changes in our bodies when we hear sudden noises",
            "answerId": "",
            "optionId": "",
            "optionIds": "6772",
            "sectionId": "1493",
            "questionNo": 16,
            "answerValue": "",
            "answerExplain": "题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。",
            "firstQuestion": false
          },
          {
            "id": "13093",
            "desc": "",
            "sort": "17",
            "title": "17 evidence that a relatively quiet environment can be more beneficial than a totally silent one",
            "answerId": "",
            "optionId": "",
            "optionIds": "6778",
            "sectionId": "1493",
            "questionNo": 17,
            "answerValue": "",
            "answerExplain": "题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。",
            "firstQuestion": false
          }
        ],
        "page_title": "Questions14 - 17",
        "part_title": "Passage 2",
        "page_content": "<br><strong class=\"mock-question-number\">14</strong> examples of strategies to decrease the noise that the public are exposed to&nbsp;#####-14-##### \n<br> \n<strong class=\"mock-question-number\">15</strong> data indicating the extent of the problem of excessive noise&nbsp;#####-15-##### \n<br> \n<strong class=\"mock-question-number\">16</strong> a description of physiological changes in our bodies when we hear sudden noises&nbsp;#####-16-##### \n<br> \n<strong class=\"mock-question-number\">17</strong> evidence that a relatively quiet environment can be more beneficial than a totally&nbsp;silent one&nbsp;#####-17-#####",
        "part_content": "<div style=\"text-align:center;\"><span style=\"font-size:18px;\"><strong>Why we need silence</strong></span></div><p><br><strong>A</strong> Humans are finely attuned to noise, and for good reason. From an evolutionary&nbsp;perspective, sounds give us vital information, helping us navigate the world and avoid danger. To help ensure loud or unexpected noises get the attention they deserve, our internal chemistry alters in response to them. Our blood pressure goes up, muscles&nbsp;tense and glands release hormones that prepare us for fight or flight. In the short term, this is a good thing. When we are exposed to too much noise over the long term, however, those responses can lead to a multitude of health issues, from sleep disturbance to even cardiovascular disease.&nbsp;<br><br><strong>B</strong> The World Health Organization has recently designated excessive noise as an 'underestimated threat' to public health, and has said that people living in cities such as Mumbai, Tokyo and Buenos Aires are being exposed to far more than the recommended 40 decibels of noise at night. A report from the European Environment Agency concluded that noise was an ongoing and widespread issue in Europe, with at least 1 in 5 people consistently exposed to levels considered harmful to health. 'There are no \"earlids\" that can protect your brain from noise,' says Nick Antonio, an acoustic consultant who has contributed to the British and international standards for noise.&nbsp;<br><br><strong>C</strong> The good news is that several cities have been working to turn the volume down. One of the first to do so was London. 'By providing recommendations for quieter buses, reducing noise from roads and also controlling noise from aircraft, they were able to make the city quieter,' says Antonio. Other cities have introduced noise-reducing road coatings, for instance, alongside greenery that muffles sound. Some solutions are more specific: Washington DC's ban on petrol-powered leaf blowers came into effect recently, while in New York City, legislation has been approved to fine people who modify their vehicles to make them noisier. 'People are seeing the benefits of these more quiet environments in their cities,' Antonio says. 'I expect we will see much more of this in the future.'&nbsp;<br><br><strong>D</strong> Researchers are also seeking to understand what aspects of silent experiences are most beneficial to our health. One of the best-researched is the flotation tank: a lightproof, soundproof tank of salt water in which a person floats as a form of deep relaxation. While some people experience altered perception in the tanks, involving subtle humming sounds and visual effects, these effects are benign and do not detract from the benefits of the experience, says Justin Feinstein, a clinical neuropsychologist. 'When you don't have external sensory stimuli coming in, the brain tries to fill the void to make sense of this dark and silent world,' he explains. 'In these tanks, some people can even hear the sound of their eyes blinking,' says Feinstein. 'But it is the ability to focus on the breath that helps people reach a relaxed or meditative state.'&nbsp;<br><br><strong>E </strong>To further explore flotation tanks as a therapeutic tool, Feinstein and his colleagues recruited 50 people with a variety of conditions related to stress and had them answer a questionnaire prior to and following a flotation session. Participants reported decreases in muscle tension, pain and symptoms of their conditions after a single, 1-hour float, alongside an increase in feelings of relaxation and overall wellbeing.&nbsp;<br><br>Less is known about what effects sensory deprivation can have on the brain. To investigate, Feinstein's team had 48 people participate in either three 90-minute float sessions or three 90-minute periods of relaxing on a chair which reclined. Participants had their brains scanned using functional magnetic resonance imaging at the beginning and end of the trial. Float sessions uniquely decreased activity in the default mode network (DMN), a collection of brain regions commonly linked with depression. Feinstein says it is an exciting finding, because flotation tanks seem to offer a way of 'resetting' our nervous system to prevent it from getting out of balance.&nbsp;<br><br><strong>F </strong>Neurobiologist Tal Dotan Ben-Soussan is also an advocate of silence as a therapy. 'When we find ways to be quiet, we are not only quiet in our environment, but quiet in our inner selves,' she explains. 'This allows us to be more aware of what is happening around us and what the situation may require from us so we can provide [a more] adequate response.'&nbsp;<br><br>Not everyone will benefit from silence to the same extent, but Ben-Soussan says one characteristic is key: the person must need to want to engage in the experience. 'We see from animal models and human studies that volition and intentionality is important,' she says. 'When people do not want silence, it can be very distressing.'&nbsp;<br><br><strong>G&nbsp;</strong>Eric Pfeifer, a psychotherapy researcher, also concedes that some people may not benefit from silence, particularly those who are in a heightened state of stress. 'People in these states may not be able to relax or calm down in a silent condition,' he says. Professional guidance can be useful, he adds, allowing people to approach silence slowly so that they can gradually enjoy the benefits. And Pfeifer is convinced that silence is more attainable in everyday life than people think. First, complete silence isn't necessary. In a recent study, he found that participants reported more relaxation and less boredom when they sat quietly in an outdoor garden compared with a completely silent room. Second, Pfeifer believes we don't need a lot of silence to gain benefits. 'You don't need to spend hours in silence,' he says. 'It is likely better to have more frequency of silence for a few minutes at a time than a longer period of silence only once a week. Just finding those places in your daily life where you can find some silence … can make a big difference.'</p>",
        "section_desc": null,
        "writing_list": [],
        "section_title": "14 examples of strategies to decrease the noise that the public are exposed to"
      },
      "options": [
        {
          "id": 6772,
          "title": "A",
          "legend": 0,
          "sortOrder": 0
        },
        {
          "id": 6773,
          "title": "B",
          "legend": 0,
          "sortOrder": 1
        },
        {
          "id": 6774,
          "title": "C",
          "legend": 0,
          "sortOrder": 2
        },
        {
          "id": 6775,
          "title": "D",
          "legend": 0,
          "sortOrder": 3
        },
        {
          "id": 6776,
          "title": "E",
          "legend": 0,
          "sortOrder": 4
        },
        {
          "id": 6777,
          "title": "F",
          "legend": 0,
          "sortOrder": 5
        },
        {
          "id": 6778,
          "title": "G",
          "legend": 0,
          "sortOrder": 6
        }
      ],
      "sort_order": 1
    },
    {
      "id": "73ce1a6e-ee97-40e9-9b06-b16ae9bf837d",
      "section_id": "67ea5018-808f-4a0b-85a9-80da99c6beba",
      "question_number_start": 18,
      "question_number_end": 21,
      "question_type": "11",
      "prompt": "Questions 18–21",
      "instruction": "<p>Complete the summary below.<br>Choose <strong>ONE WORD ONLY</strong> from the passage for each answer.<br>Write your answers in boxes 18–21 on your answer sheet.</p>",
      "content": {
        "page_desc": "<p>Complete the summary below.<br>Choose <strong>ONE WORD ONLY</strong> from the passage for each answer.<br>Write your answers in boxes 18–21 on your answer sheet.</p>",
        "questions": [
          {
            "id": "13094",
            "desc": "",
            "sort": "18",
            "title": "18.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6779",
            "sectionId": "1494",
            "questionNo": 18,
            "answerValue": "",
            "answerExplain": "题干说concentrate on（专注于）什么帮助放松和冥想。D段说focus on the breath（专注于呼吸）helps people reach a relaxed or meditative state（达到放松或冥想状态）。focus on = concentrate on。填breath。\n题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。\n题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。\n题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。",
            "firstQuestion": false
          },
          {
            "id": "13095",
            "desc": "",
            "sort": "19",
            "title": "19.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6780",
            "sectionId": "1494",
            "questionNo": 19,
            "answerValue": "",
            "answerExplain": "题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。",
            "firstQuestion": false
          },
          {
            "id": "13096",
            "desc": "",
            "sort": "20",
            "title": "20.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6781",
            "sectionId": "1494",
            "questionNo": 20,
            "answerValue": "",
            "answerExplain": "题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。",
            "firstQuestion": false
          },
          {
            "id": "13097",
            "desc": "",
            "sort": "21",
            "title": "21.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6782",
            "sectionId": "1494",
            "questionNo": 21,
            "answerValue": "",
            "answerExplain": "题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。",
            "firstQuestion": false
          }
        ],
        "page_title": "Questions 18–21",
        "part_title": "Passage 2",
        "page_content": "<div style=\"text-align:center;\"><br><span style=\"font-size:22px;\"><strong>Flotation Tanks</strong>&nbsp;</span><br>&nbsp;</div><p>According to Justin Feinstein, flotation tanks allow people to concentrate on their own&nbsp;<span data-question-id=\"18\">#####-18-#####</span>&nbsp; , which helps them relax and enables them to meditate.&nbsp;<br><br>Feinstein and his colleagues conducted an experiment in which 50 people, who were&nbsp;all suffering from stress and related issues, were given a&nbsp;<span data-question-id=\"19\">#####-19-#####</span>&nbsp;to&nbsp;complete before and after using a flotation tank. Participants reported a reduction&nbsp;in their symptoms after an hour in the tank, together with signs of relaxation and&nbsp;improved general&nbsp;<span data-question-id=\"20\">#####-20-#####</span>&nbsp;.&nbsp;<br><br>In another experiment, the researchers had 48 people spend periods of 90 minutes&nbsp;either lying back in a chair or floating in a tank. Brain scans then revealed that those&nbsp;people who had been in a tank had decreased activity in parts of the brain associated with&nbsp;<span data-question-id=\"21\">#####-21-#####</span>&nbsp;.</p>",
        "part_content": "<div style=\"text-align:center;\"><span style=\"font-size:18px;\"><strong>Why we need silence</strong></span></div><p><br><strong>A</strong> Humans are finely attuned to noise, and for good reason. From an evolutionary&nbsp;perspective, sounds give us vital information, helping us navigate the world and avoid danger. To help ensure loud or unexpected noises get the attention they deserve, our internal chemistry alters in response to them. Our blood pressure goes up, muscles&nbsp;tense and glands release hormones that prepare us for fight or flight. In the short term, this is a good thing. When we are exposed to too much noise over the long term, however, those responses can lead to a multitude of health issues, from sleep disturbance to even cardiovascular disease.&nbsp;<br><br><strong>B</strong> The World Health Organization has recently designated excessive noise as an 'underestimated threat' to public health, and has said that people living in cities such as Mumbai, Tokyo and Buenos Aires are being exposed to far more than the recommended 40 decibels of noise at night. A report from the European Environment Agency concluded that noise was an ongoing and widespread issue in Europe, with at least 1 in 5 people consistently exposed to levels considered harmful to health. 'There are no \"earlids\" that can protect your brain from noise,' says Nick Antonio, an acoustic consultant who has contributed to the British and international standards for noise.&nbsp;<br><br><strong>C</strong> The good news is that several cities have been working to turn the volume down. One of the first to do so was London. 'By providing recommendations for quieter buses, reducing noise from roads and also controlling noise from aircraft, they were able to make the city quieter,' says Antonio. Other cities have introduced noise-reducing road coatings, for instance, alongside greenery that muffles sound. Some solutions are more specific: Washington DC's ban on petrol-powered leaf blowers came into effect recently, while in New York City, legislation has been approved to fine people who modify their vehicles to make them noisier. 'People are seeing the benefits of these more quiet environments in their cities,' Antonio says. 'I expect we will see much more of this in the future.'&nbsp;<br><br><strong>D</strong> Researchers are also seeking to understand what aspects of silent experiences are most beneficial to our health. One of the best-researched is the flotation tank: a lightproof, soundproof tank of salt water in which a person floats as a form of deep relaxation. While some people experience altered perception in the tanks, involving subtle humming sounds and visual effects, these effects are benign and do not detract from the benefits of the experience, says Justin Feinstein, a clinical neuropsychologist. 'When you don't have external sensory stimuli coming in, the brain tries to fill the void to make sense of this dark and silent world,' he explains. 'In these tanks, some people can even hear the sound of their eyes blinking,' says Feinstein. 'But it is the ability to focus on the breath that helps people reach a relaxed or meditative state.'&nbsp;<br><br><strong>E </strong>To further explore flotation tanks as a therapeutic tool, Feinstein and his colleagues recruited 50 people with a variety of conditions related to stress and had them answer a questionnaire prior to and following a flotation session. Participants reported decreases in muscle tension, pain and symptoms of their conditions after a single, 1-hour float, alongside an increase in feelings of relaxation and overall wellbeing.&nbsp;<br><br>Less is known about what effects sensory deprivation can have on the brain. To investigate, Feinstein's team had 48 people participate in either three 90-minute float sessions or three 90-minute periods of relaxing on a chair which reclined. Participants had their brains scanned using functional magnetic resonance imaging at the beginning and end of the trial. Float sessions uniquely decreased activity in the default mode network (DMN), a collection of brain regions commonly linked with depression. Feinstein says it is an exciting finding, because flotation tanks seem to offer a way of 'resetting' our nervous system to prevent it from getting out of balance.&nbsp;<br><br><strong>F </strong>Neurobiologist Tal Dotan Ben-Soussan is also an advocate of silence as a therapy. 'When we find ways to be quiet, we are not only quiet in our environment, but quiet in our inner selves,' she explains. 'This allows us to be more aware of what is happening around us and what the situation may require from us so we can provide [a more] adequate response.'&nbsp;<br><br>Not everyone will benefit from silence to the same extent, but Ben-Soussan says one characteristic is key: the person must need to want to engage in the experience. 'We see from animal models and human studies that volition and intentionality is important,' she says. 'When people do not want silence, it can be very distressing.'&nbsp;<br><br><strong>G&nbsp;</strong>Eric Pfeifer, a psychotherapy researcher, also concedes that some people may not benefit from silence, particularly those who are in a heightened state of stress. 'People in these states may not be able to relax or calm down in a silent condition,' he says. Professional guidance can be useful, he adds, allowing people to approach silence slowly so that they can gradually enjoy the benefits. And Pfeifer is convinced that silence is more attainable in everyday life than people think. First, complete silence isn't necessary. In a recent study, he found that participants reported more relaxation and less boredom when they sat quietly in an outdoor garden compared with a completely silent room. Second, Pfeifer believes we don't need a lot of silence to gain benefits. 'You don't need to spend hours in silence,' he says. 'It is likely better to have more frequency of silence for a few minutes at a time than a longer period of silence only once a week. Just finding those places in your daily life where you can find some silence … can make a big difference.'</p>",
        "section_desc": null,
        "writing_list": [],
        "section_title": "18-21."
      },
      "options": [],
      "sort_order": 2
    },
    {
      "id": "ebd4dafd-42bc-440b-b9fa-18a0bb8de329",
      "section_id": "67ea5018-808f-4a0b-85a9-80da99c6beba",
      "question_number_start": 22,
      "question_number_end": 26,
      "question_type": "6",
      "prompt": "Questions 22–26",
      "instruction": "<p><i>Look at the following statements (Questions 22–26) and the list of people below.</i><br><i>Match each statement with the correct person, <strong>A</strong>, <strong>B</strong>, <strong>C</strong> or <strong>D</strong>.</i><br><i>Write the correct letter, <strong>A</strong>, <strong>B</strong>, <strong>C</strong> or <strong>D</strong>, in boxes 22–26 on your answer sheet.</i><br><i><strong>NB</strong> You may use any letter more than once.</i></p>",
      "content": {
        "page_desc": "<p><i>Look at the following statements (Questions 22–26) and the list of people below.</i><br><i>Match each statement with the correct person, <strong>A</strong>, <strong>B</strong>, <strong>C</strong> or <strong>D</strong>.</i><br><i>Write the correct letter, <strong>A</strong>, <strong>B</strong>, <strong>C</strong> or <strong>D</strong>, in boxes 22–26 on your answer sheet.</i><br><i><strong>NB</strong> You may use any letter more than once.</i></p>",
        "questions": [
          {
            "id": "13098",
            "desc": "",
            "sort": "22",
            "title": "22 It is unpleasant and upsetting for people to be placed in a silent environment against their will.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6785",
            "sectionId": "1495",
            "questionNo": 22,
            "answerValue": "",
            "answerExplain": "题干说违背意愿被置于安静环境中让人unpleasant and upsetting（不愉快和痛苦）。F段Ben-Soussan说When people do not want silence, it can be very distressing（不想要的安静令人痛苦）。完全对应。选C。\n题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。\n题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。\n题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。\n题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）",
            "firstQuestion": false
          },
          {
            "id": "13099",
            "desc": "",
            "sort": "23",
            "title": "23 The trend towards creating quieter urban locations is likely to increase in the coming years.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6783",
            "sectionId": "1495",
            "questionNo": 23,
            "answerValue": "",
            "answerExplain": "题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。",
            "firstQuestion": false
          },
          {
            "id": "13100",
            "desc": "",
            "sort": "24",
            "title": "24 When our body's senses are completely deprived of input, our minds compensate for this by creating the illusion of images and sounds.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6784",
            "sectionId": "1495",
            "questionNo": 24,
            "answerValue": "",
            "answerExplain": "题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。",
            "firstQuestion": false
          },
          {
            "id": "13101",
            "desc": "",
            "sort": "25",
            "title": "25 Even a short amount of silent time can have a positive impact.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6786",
            "sectionId": "1495",
            "questionNo": 25,
            "answerValue": "",
            "answerExplain": "题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。",
            "firstQuestion": false
          },
          {
            "id": "13102",
            "desc": "",
            "sort": "26",
            "title": "26 External and internal quietness makes us more conscious of events occurring in our surroundings and helps us react appropriately to these events.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6785",
            "sectionId": "1495",
            "questionNo": 26,
            "answerValue": "",
            "answerExplain": "题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）",
            "firstQuestion": false
          }
        ],
        "page_title": "Questions 22–26",
        "part_title": "Passage 2",
        "page_content": "<p style=\"text-align:center;\"><strong>List of People</strong></p>",
        "part_content": "",
        "section_desc": "",
        "writing_list": [],
        "section_title": "22 It is unpleasant and upsetting for people to be placed in a silent environment against their will."
      },
      "options": [
        {
          "id": 6783,
          "title": "A. Nick Antonio",
          "legend": 0,
          "sortOrder": 0
        },
        {
          "id": 6784,
          "title": "B. Justin Feinstein",
          "legend": 0,
          "sortOrder": 1
        },
        {
          "id": 6785,
          "title": "C. Tal Dotan Ben-Soussan",
          "legend": 0,
          "sortOrder": 2
        },
        {
          "id": 6786,
          "title": "D. Eric Pfeifer",
          "legend": 0,
          "sortOrder": 3
        }
      ],
      "sort_order": 3
    }
  ]
answers_json: |
  [
    {
      "id": "f2ce7600-9c9e-4ea9-a55d-6202de65d270",
      "question_id": "2961d822-6527-4849-b40c-161aa09bb111",
      "answer_data": {
        "answers": [
          {
            "title": "14 examples of strategies to decrease the noise that the public are exposed to",
            "answer_id": null,
            "option_ids": "6774",
            "question_no": 14,
            "answer_value": null,
            "answer_explain": "题干关键词strategies to decrease noise the public are exposed to（降低公众噪音暴露的策略）。C段首句说several cities have been working to turn the volume down（调低音量）。整段列举quiet buses、降噪路面、leaf blower禁令等措施。选C。\n题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。\n题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。\n题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。"
          },
          {
            "title": "15 data indicating the extent of the problem of excessive noise",
            "answer_id": null,
            "option_ids": "6773",
            "question_no": 15,
            "answer_value": null,
            "answer_explain": "题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。"
          },
          {
            "title": "16 a description of physiological changes in our bodies when we hear sudden noises",
            "answer_id": null,
            "option_ids": "6772",
            "question_no": 16,
            "answer_value": null,
            "answer_explain": "题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。"
          },
          {
            "title": "17 evidence that a relatively quiet environment can be more beneficial than a totally silent one",
            "answer_id": null,
            "option_ids": "6778",
            "question_no": 17,
            "answer_value": null,
            "answer_explain": "题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。"
          }
        ],
        "options": [
          {
            "id": 6772,
            "title": "A",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 6773,
            "title": "B",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 6774,
            "title": "C",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 6775,
            "title": "D",
            "legend": 0,
            "sortOrder": 3
          },
          {
            "id": 6776,
            "title": "E",
            "legend": 0,
            "sortOrder": 4
          },
          {
            "id": 6777,
            "title": "F",
            "legend": 0,
            "sortOrder": 5
          },
          {
            "id": 6778,
            "title": "G",
            "legend": 0,
            "sortOrder": 6
          }
        ]
      },
      "explanation": "题干关键词strategies to decrease noise the public are exposed to（降低公众噪音暴露的策略）。C段首句说several cities have been working to turn the volume down（调低音量）。整段列举quiet buses、降噪路面、leaf blower禁令等措施。选C。\n题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。\n题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。\n题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。\n\n题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。\n\n题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。\n\n题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。",
      "raw_data": {
        "id": "1493",
        "res": "",
        "desc": "",
        "type": "28",
        "title": "14 examples of strategies to decrease the noise that the public are exposed to",
        "partId": 476,
        "options": [
          {
            "id": 6772,
            "title": "A",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 6773,
            "title": "B",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 6774,
            "title": "C",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 6775,
            "title": "D",
            "legend": 0,
            "sortOrder": 3
          },
          {
            "id": 6776,
            "title": "E",
            "legend": 0,
            "sortOrder": 4
          },
          {
            "id": 6777,
            "title": "F",
            "legend": 0,
            "sortOrder": 5
          },
          {
            "id": 6778,
            "title": "G",
            "legend": 0,
            "sortOrder": 6
          }
        ],
        "paperId": 53,
        "classify": "2",
        "realType": "7",
        "chapterId": "1237",
        "questions": [
          {
            "id": "13090",
            "desc": "",
            "sort": "14",
            "title": "14 examples of strategies to decrease the noise that the public are exposed to",
            "answerId": "",
            "optionId": "",
            "optionIds": "6774",
            "sectionId": "1493",
            "questionNo": 14,
            "answerValue": "",
            "answerExplain": "题干关键词strategies to decrease noise the public are exposed to（降低公众噪音暴露的策略）。C段首句说several cities have been working to turn the volume down（调低音量）。整段列举quiet buses、降噪路面、leaf blower禁令等措施。选C。\n题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。\n题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。\n题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。",
            "firstQuestion": false
          },
          {
            "id": "13091",
            "desc": "",
            "sort": "15",
            "title": "15 data indicating the extent of the problem of excessive noise",
            "answerId": "",
            "optionId": "",
            "optionIds": "6773",
            "sectionId": "1493",
            "questionNo": 15,
            "answerValue": "",
            "answerExplain": "题干问data indicating the extent of the problem（显示问题严重程度的数据）。B段给出两组数据：WHO说城市噪音远超40分贝，欧洲环境署说至少1/5人口暴露在有害噪音下。其他段没有量化数据。选B。",
            "firstQuestion": false
          },
          {
            "id": "13092",
            "desc": "",
            "sort": "16",
            "title": "16 a description of physiological changes in our bodies when we hear sudden noises",
            "answerId": "",
            "optionId": "",
            "optionIds": "6772",
            "sectionId": "1493",
            "questionNo": 16,
            "answerValue": "",
            "answerExplain": "题干问physiological changes when we hear sudden noises（听到突然噪音时的生理变化）。A段说blood pressure goes up, muscles tense, glands release hormones（血压上升、肌肉紧绷、腺体释放激素）——全是生理变化。选A。",
            "firstQuestion": false
          },
          {
            "id": "13093",
            "desc": "",
            "sort": "17",
            "title": "17 evidence that a relatively quiet environment can be more beneficial than a totally silent one",
            "answerId": "",
            "optionId": "",
            "optionIds": "6778",
            "sectionId": "1493",
            "questionNo": 17,
            "answerValue": "",
            "answerExplain": "题干说relatively quiet（相对安静）比totally silent（完全安静）更有益。G段直接比较outdoor garden vs completely silent room，前者带来more relaxation（更多放松）。选G。",
            "firstQuestion": false
          }
        ]
      }
    },
    {
      "id": "3721e727-589f-4a8b-80c1-73d06e874cc8",
      "question_id": "73ce1a6e-ee97-40e9-9b06-b16ae9bf837d",
      "answer_data": {
        "answers": [
          {
            "title": "18.",
            "answer_id": null,
            "option_ids": "6779",
            "question_no": 18,
            "answer_value": null,
            "answer_explain": "题干说concentrate on（专注于）什么帮助放松和冥想。D段说focus on the breath（专注于呼吸）helps people reach a relaxed or meditative state（达到放松或冥想状态）。focus on = concentrate on。填breath。\n题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。\n题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。\n题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。"
          },
          {
            "title": "19.",
            "answer_id": null,
            "option_ids": "6780",
            "question_no": 19,
            "answer_value": null,
            "answer_explain": "题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。"
          },
          {
            "title": "20.",
            "answer_id": null,
            "option_ids": "6781",
            "question_no": 20,
            "answer_value": null,
            "answer_explain": "题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。"
          },
          {
            "title": "21.",
            "answer_id": null,
            "option_ids": "6782",
            "question_no": 21,
            "answer_value": null,
            "answer_explain": "题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。"
          }
        ],
        "options": [
          {
            "id": 6779,
            "title": "breath",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 6780,
            "title": "questionnaire",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 6781,
            "title": "wellbeing",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 6782,
            "title": "depression",
            "legend": 0,
            "sortOrder": 3
          }
        ]
      },
      "explanation": "题干说concentrate on（专注于）什么帮助放松和冥想。D段说focus on the breath（专注于呼吸）helps people reach a relaxed or meditative state（达到放松或冥想状态）。focus on = concentrate on。填breath。\n题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。\n题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。\n题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。\n\n题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。\n\n题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。\n\n题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。",
      "raw_data": {
        "id": "1494",
        "res": "",
        "desc": "",
        "type": "10",
        "title": "18-21.",
        "partId": 476,
        "options": [
          {
            "id": 6779,
            "title": "breath",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 6780,
            "title": "questionnaire",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 6781,
            "title": "wellbeing",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 6782,
            "title": "depression",
            "legend": 0,
            "sortOrder": 3
          }
        ],
        "paperId": 53,
        "classify": "2",
        "realType": "11",
        "chapterId": "1238",
        "questions": [
          {
            "id": "13094",
            "desc": "",
            "sort": "18",
            "title": "18.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6779",
            "sectionId": "1494",
            "questionNo": 18,
            "answerValue": "",
            "answerExplain": "题干说concentrate on（专注于）什么帮助放松和冥想。D段说focus on the breath（专注于呼吸）helps people reach a relaxed or meditative state（达到放松或冥想状态）。focus on = concentrate on。填breath。\n题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。\n题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。\n题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。",
            "firstQuestion": false
          },
          {
            "id": "13095",
            "desc": "",
            "sort": "19",
            "title": "19.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6780",
            "sectionId": "1494",
            "questionNo": 19,
            "answerValue": "",
            "answerExplain": "题干说50个人在漂浮前后（before and after）完成什么。E段说had them answer a questionnaire prior to and following a flotation session（漂浮前后填写问卷）。prior to and following = before and after。填questionnaire。",
            "firstQuestion": false
          },
          {
            "id": "13096",
            "desc": "",
            "sort": "20",
            "title": "20.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6781",
            "sectionId": "1494",
            "questionNo": 20,
            "answerValue": "",
            "answerExplain": "题干说提升了的general（总体）什么。E段说increase in feelings of relaxation and overall wellbeing（放松感和整体幸福感提升）。overall = general，wellbeing = 幸福感。填wellbeing。",
            "firstQuestion": false
          },
          {
            "id": "13097",
            "desc": "",
            "sort": "21",
            "title": "21.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6782",
            "sectionId": "1494",
            "questionNo": 21,
            "answerValue": "",
            "answerExplain": "题干说脑部活动减少的区域与什么有关。E段说decreased activity in DMN……linked with depression（与抑郁相关）。填depression。",
            "firstQuestion": false
          }
        ]
      }
    },
    {
      "id": "e34a5835-09b3-4233-a290-993db5755dc6",
      "question_id": "ebd4dafd-42bc-440b-b9fa-18a0bb8de329",
      "answer_data": {
        "answers": [
          {
            "title": "22 It is unpleasant and upsetting for people to be placed in a silent environment against their will.",
            "answer_id": null,
            "option_ids": "6785",
            "question_no": 22,
            "answer_value": null,
            "answer_explain": "题干说违背意愿被置于安静环境中让人unpleasant and upsetting（不愉快和痛苦）。F段Ben-Soussan说When people do not want silence, it can be very distressing（不想要的安静令人痛苦）。完全对应。选C。\n题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。\n题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。\n题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。\n题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）"
          },
          {
            "title": "23 The trend towards creating quieter urban locations is likely to increase in the coming years.",
            "answer_id": null,
            "option_ids": "6783",
            "question_no": 23,
            "answer_value": null,
            "answer_explain": "题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。"
          },
          {
            "title": "24 When our body's senses are completely deprived of input, our minds compensate for this by creating the illusion of images and sounds.",
            "answer_id": null,
            "option_ids": "6784",
            "question_no": 24,
            "answer_value": null,
            "answer_explain": "题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。"
          },
          {
            "title": "25 Even a short amount of silent time can have a positive impact.",
            "answer_id": null,
            "option_ids": "6786",
            "question_no": 25,
            "answer_value": null,
            "answer_explain": "题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。"
          },
          {
            "title": "26 External and internal quietness makes us more conscious of events occurring in our surroundings and helps us react appropriately to these events.",
            "answer_id": null,
            "option_ids": "6785",
            "question_no": 26,
            "answer_value": null,
            "answer_explain": "题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）"
          }
        ],
        "options": [
          {
            "id": 6783,
            "title": "A. Nick Antonio",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 6784,
            "title": "B. Justin Feinstein",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 6785,
            "title": "C. Tal Dotan Ben-Soussan",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 6786,
            "title": "D. Eric Pfeifer",
            "legend": 0,
            "sortOrder": 3
          }
        ]
      },
      "explanation": "题干说违背意愿被置于安静环境中让人unpleasant and upsetting（不愉快和痛苦）。F段Ben-Soussan说When people do not want silence, it can be very distressing（不想要的安静令人痛苦）。完全对应。选C。\n题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。\n题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。\n题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。\n题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）\n\n题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。\n\n题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。\n\n题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。\n\n题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）",
      "raw_data": {
        "id": "1495",
        "res": "",
        "desc": "List of People",
        "type": "22",
        "title": "22 It is unpleasant and upsetting for people to be placed in a silent environment against their will.",
        "partId": 476,
        "options": [
          {
            "id": 6783,
            "title": "A. Nick Antonio",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 6784,
            "title": "B. Justin Feinstein",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 6785,
            "title": "C. Tal Dotan Ben-Soussan",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 6786,
            "title": "D. Eric Pfeifer",
            "legend": 0,
            "sortOrder": 3
          }
        ],
        "paperId": 53,
        "classify": "2",
        "realType": "6",
        "chapterId": "1239",
        "questions": [
          {
            "id": "13098",
            "desc": "",
            "sort": "22",
            "title": "22 It is unpleasant and upsetting for people to be placed in a silent environment against their will.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6785",
            "sectionId": "1495",
            "questionNo": 22,
            "answerValue": "",
            "answerExplain": "题干说违背意愿被置于安静环境中让人unpleasant and upsetting（不愉快和痛苦）。F段Ben-Soussan说When people do not want silence, it can be very distressing（不想要的安静令人痛苦）。完全对应。选C。\n题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。\n题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。\n题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。\n题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）",
            "firstQuestion": false
          },
          {
            "id": "13099",
            "desc": "",
            "sort": "23",
            "title": "23 The trend towards creating quieter urban locations is likely to increase in the coming years.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6783",
            "sectionId": "1495",
            "questionNo": 23,
            "answerValue": "",
            "answerExplain": "题干说创建更安静城市地点的趋势may increase（可能增加）。C段Antonio列举措施后说I expect we will see much more of this in the future（未来会出现更多）。对应A。",
            "firstQuestion": false
          },
          {
            "id": "13100",
            "desc": "",
            "sort": "24",
            "title": "24 When our body's senses are completely deprived of input, our minds compensate for this by creating the illusion of images and sounds.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6784",
            "sectionId": "1495",
            "questionNo": 24,
            "answerValue": "",
            "answerExplain": "题干说sensory deprivation（感官被剥夺）时大脑通过创造幻觉来补偿。D段Feinstein说When you don't have external sensory stimuli（没有外部刺激），the brain tries to fill the void（大脑填补空白）。对应B。",
            "firstQuestion": false
          },
          {
            "id": "13101",
            "desc": "",
            "sort": "25",
            "title": "25 Even a short amount of silent time can have a positive impact.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6786",
            "sectionId": "1495",
            "questionNo": 25,
            "answerValue": "",
            "answerExplain": "题干说少量安静时间也有积极影响。G段Pfeifer说You don't need to spend hours……a few minutes at a time（不需要几小时，几分钟就够了）。对应D。",
            "firstQuestion": false
          },
          {
            "id": "13102",
            "desc": "",
            "sort": "26",
            "title": "26 External and internal quietness makes us more conscious of events occurring in our surroundings and helps us react appropriately to these events.",
            "answerId": "",
            "optionId": "",
            "optionIds": "6785",
            "sectionId": "1495",
            "questionNo": 26,
            "answerValue": "",
            "answerExplain": "题干说内部和外部安静让我们更清楚周围的事并做出恰当反应。F段Ben-Soussan说more aware of what is happening around us（更清楚周围）……provide adequate response（做出充分回应）。对应C。 Passage 3 Book review: The World of Sugar by Ulbe Bosma（书评：《糖的世界》）",
            "firstQuestion": false
          }
        ]
      }
    }
  ]
assets_json: |
  [
    {
      "id": "e2372245-d6a8-4f6a-b7fe-84121822a76c",
      "book_id": "1249e423-5c10-498d-ac08-2e90df646c5f",
      "test_id": "92705880-e1f1-4d09-89b2-45652415323f",
      "module_id": "a1736f94-930d-458e-beb8-c4e4d5d1c496",
      "section_id": null,
      "question_id": null,
      "asset_type": "json",
      "bucket": "ielts",
      "storage_path": "21/test-1/reading/raw.json",
      "public_url": "https://xnmlzimdpawdqikgbpvw.supabase.co/storage/v1/object/public/ielts/21/test-1/reading/raw.json",
      "mime_type": "application/json",
      "duration_seconds": null,
      "metadata": {
        "classify": 2,
        "source_url": "https://www.winielts.com/api/paper/exam/queryByIdAndClassify?id=53&classify=2&type=1",
        "source_paper_id": 53
      }
    }
  ]
passage_translation_json: |
  [
    {
      "id": "s1",
      "en": "Why we need silence",
      "zh": "我们为什么需要安静"
    },
    {
      "id": "s2",
      "en": "A Humans are finely attuned to noise, and for good reason.",
      "zh": "A 人类对噪音非常敏感，这是有充分理由的。"
    },
    {
      "id": "s3",
      "en": "From an evolutionary perspective, sounds give us vital information, helping us navigate the world and avoid danger.",
      "zh": "从进化的角度来看，声音为我们提供了重要的信息，帮助我们在世界中导航并避免危险。"
    },
    {
      "id": "s4",
      "en": "To help ensure loud or unexpected noises get the attention they deserve, our internal chemistry alters in response to them.",
      "zh": "为了确保响亮或意外的声音能够引起我们的注意，我们的体内化学反应会随之改变。"
    },
    {
      "id": "s5",
      "en": "Our blood pressure goes up, muscles tense and glands release hormones that prepare us for fight or flight.",
      "zh": "我们的血压升高，肌肉绷紧，腺体释放激素，为战斗或逃跑做好准备。"
    },
    {
      "id": "s6",
      "en": "In the short term, this is a good thing.",
      "zh": "从短期来看，这是好事。"
    },
    {
      "id": "s7",
      "en": "When we are exposed to too much noise over the long term, however, those responses can lead to a multitude of health issues, from sleep disturbance to even cardiovascular disease.",
      "zh": "然而，长期暴露于过多噪音中，这些反应可能导致多种健康问题，从睡眠障碍到心血管疾病。"
    },
    {
      "id": "s8",
      "en": "B The World Health Organization has recently designated excessive noise as an 'underestimated threat' to public health, and has said that people living in cities such as Mumbai, Tokyo and Buenos Aires are being exposed to far more than the recommended 40 decibels of noise at night.",
      "zh": "B 世界卫生组织最近将过度噪音列为公共健康的“被低估的威胁”，并指出生活在孟买、东京和布宜诺斯艾利斯等城市的人们夜间暴露的噪音远远超过了建议的40分贝。"
    },
    {
      "id": "s9",
      "en": "A report from the European Environment Agency concluded that noise was an ongoing and widespread issue in Europe, with at least 1 in 5 people consistently exposed to levels considered harmful to health.",
      "zh": "欧洲环境署的一份报告得出结论，噪音在欧洲是一个持续且广泛存在的问题，至少五分之一的人长期暴露于被认为有害健康的噪音水平。"
    },
    {
      "id": "s10",
      "en": "'There are no \"earlids\" that can protect your brain from noise,' says Nick Antonio, an acoustic consultant who has contributed to the British and international standards for noise.",
      "zh": "曾为英国和国际噪声标准做出贡献的声学顾问尼克·安东尼奥说：“没有‘耳睑’能保护你的大脑免受噪音影响。”"
    },
    {
      "id": "s11",
      "en": "C The good news is that several cities have been working to turn the volume down.",
      "zh": "C 好消息是，几个城市一直在努力降低噪音水平。"
    },
    {
      "id": "s12",
      "en": "One of the first to do so was London.",
      "zh": "伦敦是最早采取行动的城市之一。"
    },
    {
      "id": "s13",
      "en": "'By providing recommendations for quieter buses, reducing noise from roads and also controlling noise from aircraft, they were able to make the city quieter,' says Antonio.",
      "zh": "安东尼奥说：“通过对更安静的公交车提出建议，减少道路噪音以及控制飞机噪音，他们成功使这座城市变得更安静。”"
    },
    {
      "id": "s14",
      "en": "Other cities have introduced noise-reducing road coatings, for instance, alongside greenery that muffles sound.",
      "zh": "其他城市则引进了降噪道路涂层，并种植绿植来减弱声音。"
    },
    {
      "id": "s15",
      "en": "Some solutions are more specific: Washington DC's ban on petrol-powered leaf blowers came into effect recently, while in New York City, legislation has been approved to fine people who modify their vehicles to make them noisier.",
      "zh": "有些解决方案更为具体：华盛顿特区最近开始禁止使用汽油动力的吹叶机，而纽约市也通过了立法，对改装车辆使其变得更吵的人进行罚款。"
    },
    {
      "id": "s16",
      "en": "'People are seeing the benefits of these more quiet environments in their cities,' Antonio says.",
      "zh": "安东尼奥说：“人们正在感受到他们城市中更安静环境带来的好处。”"
    },
    {
      "id": "s17",
      "en": "'I expect we will see much more of this in the future.'",
      "zh": "“我预计未来我们将看到更多这样的举措。”"
    },
    {
      "id": "s18",
      "en": "D Researchers are also seeking to understand what aspects of silent experiences are most beneficial to our health.",
      "zh": "D 研究人员还在努力了解安静体验中哪些方面对我们的健康最有益。"
    },
    {
      "id": "s19",
      "en": "One of the best-researched is the flotation tank: a lightproof, soundproof tank of salt water in which a person floats as a form of deep relaxation.",
      "zh": "研究最充分的之一是漂浮舱：一个遮光、隔音的盐水箱，人可以漂浮其中，进行深度放松。"
    },
    {
      "id": "s20",
      "en": "While some people experience altered perception in the tanks, involving subtle humming sounds and visual effects, these effects are benign and do not detract from the benefits of the experience, says Justin Feinstein, a clinical neuropsychologist.",
      "zh": "临床神经心理学家贾斯汀·费因斯坦说，尽管有些人在漂浮舱中会感受到改变的感知，包括细微的嗡嗡声和视觉效果，但这些影响是无害的，并不会削弱体验的益处。"
    },
    {
      "id": "s21",
      "en": "'When you don't have external sensory stimuli coming in, the brain tries to fill the void to make sense of this dark and silent world,' he explains.",
      "zh": "他解释道：“当你没有外部感官刺激传入时，大脑会试图填补空白，以理解这个黑暗而寂静的世界。”"
    },
    {
      "id": "s22",
      "en": "'In these tanks, some people can even hear the sound of their eyes blinking,' says Feinstein.",
      "zh": "费因斯坦说：“在这些舱内，有些人甚至能听到自己眨眼的声音。”"
    },
    {
      "id": "s23",
      "en": "'But it is the ability to focus on the breath that helps people reach a relaxed or meditative state.'",
      "zh": "“但正是专注于呼吸的能力，帮助人们达到放松或冥想的状态。”"
    },
    {
      "id": "s24",
      "en": "E To further explore flotation tanks as a therapeutic tool, Feinstein and his colleagues recruited 50 people with a variety of conditions related to stress and had them answer a questionnaire prior to and following a flotation session.",
      "zh": "E 为了进一步探索漂浮舱作为治疗工具的效果，费因斯坦和他的同事招募了50名有各种压力相关状况的人，要求他们在漂浮前后填写问卷。"
    },
    {
      "id": "s25",
      "en": "Participants reported decreases in muscle tension, pain and symptoms of their conditions after a single, 1-hour float, alongside an increase in feelings of relaxation and overall wellbeing.",
      "zh": "参与者报告称，经历一次一小时的漂浮后，肌肉紧张、疼痛及病症相关症状有所减轻，同时放松感和整体幸福感增加。"
    },
    {
      "id": "s26",
      "en": "Less is known about what effects sensory deprivation can have on the brain.",
      "zh": "对于感官剥夺对大脑可能产生的影响，人们知之甚少。"
    },
    {
      "id": "s27",
      "en": "To investigate, Feinstein's team had 48 people participate in either three 90-minute float sessions or three 90-minute periods of relaxing on a chair which reclined.",
      "zh": "为此，费因斯坦团队让48人参加了三次90分钟的漂浮体验，或三次90分钟的躺椅放松体验。"
    },
    {
      "id": "s28",
      "en": "Participants had their brains scanned using functional magnetic resonance imaging at the beginning and end of the trial.",
      "zh": "参与者在试验开始和结束时接受功能性磁共振成像（fMRI）扫描。"
    },
    {
      "id": "s29",
      "en": "Float sessions uniquely decreased activity in the default mode network (DMN), a collection of brain regions commonly linked with depression.",
      "zh": "漂浮体验独特地减少了默认模式网络（DMN）的活跃度，DMN是一组通常与抑郁相关的大脑区域。"
    },
    {
      "id": "s30",
      "en": "Feinstein says it is an exciting finding, because flotation tanks seem to offer a way of 'resetting' our nervous system to prevent it from getting out of balance.",
      "zh": "费因斯坦表示，这是一项令人兴奋的发现，因为漂浮舱似乎提供了一种“重置”我们神经系统的方法，以防止其失衡。"
    },
    {
      "id": "s31",
      "en": "F Neurobiologist Tal Dotan Ben-Soussan is also an advocate of silence as a therapy.",
      "zh": "F 神经生物学家塔尔·多坦·本-苏桑也倡导将安静作为一种疗法。"
    },
    {
      "id": "s32",
      "en": "'When we find ways to be quiet, we are not only quiet in our environment, but quiet in our inner selves,' she explains.",
      "zh": "她解释道：“当我们找到安静的方法时，我们不仅是外在环境的安静，更是内心的宁静。”"
    },
    {
      "id": "s33",
      "en": "'This allows us to be more aware of what is happening around us and what the situation may require from us so we can provide [a more] adequate response.'",
      "zh": "“这让我们更清楚周围发生的事情及情境对我们的需求，从而作出更恰当的反应。”"
    },
    {
      "id": "s34",
      "en": "Not everyone will benefit from silence to the same extent, but Ben-Soussan says one characteristic is key: the person must need to want to engage in the experience.",
      "zh": "不是每个人都能从安静中获得同样的益处，但本-苏桑说有一个关键特征：个人必须真正愿意体验这份安静。"
    },
    {
      "id": "s35",
      "en": "'We see from animal models and human studies that volition and intentionality is important,' she says.",
      "zh": "“动物模型和人体研究均显示，意愿和主动性非常重要，”她说。"
    },
    {
      "id": "s36",
      "en": "'When people do not want silence, it can be very distressing.'",
      "zh": "“当人们不想要安静时，安静可能会让他们感到非常痛苦。”"
    },
    {
      "id": "s37",
      "en": "G Eric Pfeifer, a psychotherapy researcher, also concedes that some people may not benefit from silence, particularly those who are in a heightened state of stress.",
      "zh": "G 心理治疗研究员埃里克·普法弗也承认，有些人可能无法从安静中受益，尤其是处于高度压力状态的人。"
    },
    {
      "id": "s38",
      "en": "'People in these states may not be able to relax or calm down in a silent condition,' he says.",
      "zh": "他说：“处于这些状态中的人可能无法在安静环境下放松或平静下来。”"
    },
    {
      "id": "s39",
      "en": "Professional guidance can be useful, he adds, allowing people to approach silence slowly so that they can gradually enjoy the benefits.",
      "zh": "他补充说，专业指导很有帮助，可以让人们慢慢接触安静，从而逐渐享受其益处。"
    },
    {
      "id": "s40",
      "en": "And Pfeifer is convinced that silence is more attainable in everyday life than people think.",
      "zh": "普法弗坚信，安静在日常生活中比人们想象的更容易获得。"
    },
    {
      "id": "s41",
      "en": "First, complete silence isn't necessary.",
      "zh": "首先，完全的安静并非必要。"
    },
    {
      "id": "s42",
      "en": "In a recent study, he found that participants reported more relaxation and less boredom when they sat quietly in an outdoor garden compared with a completely silent room.",
      "zh": "在一项近期研究中，他发现参与者在户外花园安静坐着时，比起在完全安静的房间里，感到更放松且更不无聊。"
    },
    {
      "id": "s43",
      "en": "Second, Pfeifer believes we don't need a lot of silence to gain benefits.",
      "zh": "其次，普法弗认为我们不需要大量的安静就能获得益处。"
    },
    {
      "id": "s44",
      "en": "'You don't need to spend hours in silence,' he says.",
      "zh": "他说：“你不需要花几个小时保持安静。”"
    },
    {
      "id": "s45",
      "en": "'It is likely better to have more frequency of silence for a few minutes at a time than a longer period of silence only once a week.",
      "zh": "“相比于每周只有一次长时间的安静，可能更好的是更频繁地每次保持几分钟的安静。”"
    },
    {
      "id": "s46",
      "en": "Just finding those places in your daily life where you can find some silence … can make a big difference.'",
      "zh": "“只要在日常生活中找到可以享受片刻安静的地方……就能带来很大变化。”"
    }
  ]
---
<div style="text-align:center;"><span style="font-size:18px;"><strong>Why we need silence</strong></span></div><p><br><strong>A</strong> Humans are finely attuned to noise, and for good reason. From an evolutionary&nbsp;perspective, sounds give us vital information, helping us navigate the world and avoid danger. To help ensure loud or unexpected noises get the attention they deserve, our internal chemistry alters in response to them. Our blood pressure goes up, muscles&nbsp;tense and glands release hormones that prepare us for fight or flight. In the short term, this is a good thing. When we are exposed to too much noise over the long term, however, those responses can lead to a multitude of health issues, from sleep disturbance to even cardiovascular disease.&nbsp;<br><br><strong>B</strong> The World Health Organization has recently designated excessive noise as an 'underestimated threat' to public health, and has said that people living in cities such as Mumbai, Tokyo and Buenos Aires are being exposed to far more than the recommended 40 decibels of noise at night. A report from the European Environment Agency concluded that noise was an ongoing and widespread issue in Europe, with at least 1 in 5 people consistently exposed to levels considered harmful to health. 'There are no "earlids" that can protect your brain from noise,' says Nick Antonio, an acoustic consultant who has contributed to the British and international standards for noise.&nbsp;<br><br><strong>C</strong> The good news is that several cities have been working to turn the volume down. One of the first to do so was London. 'By providing recommendations for quieter buses, reducing noise from roads and also controlling noise from aircraft, they were able to make the city quieter,' says Antonio. Other cities have introduced noise-reducing road coatings, for instance, alongside greenery that muffles sound. Some solutions are more specific: Washington DC's ban on petrol-powered leaf blowers came into effect recently, while in New York City, legislation has been approved to fine people who modify their vehicles to make them noisier. 'People are seeing the benefits of these more quiet environments in their cities,' Antonio says. 'I expect we will see much more of this in the future.'&nbsp;<br><br><strong>D</strong> Researchers are also seeking to understand what aspects of silent experiences are most beneficial to our health. One of the best-researched is the flotation tank: a lightproof, soundproof tank of salt water in which a person floats as a form of deep relaxation. While some people experience altered perception in the tanks, involving subtle humming sounds and visual effects, these effects are benign and do not detract from the benefits of the experience, says Justin Feinstein, a clinical neuropsychologist. 'When you don't have external sensory stimuli coming in, the brain tries to fill the void to make sense of this dark and silent world,' he explains. 'In these tanks, some people can even hear the sound of their eyes blinking,' says Feinstein. 'But it is the ability to focus on the breath that helps people reach a relaxed or meditative state.'&nbsp;<br><br><strong>E </strong>To further explore flotation tanks as a therapeutic tool, Feinstein and his colleagues recruited 50 people with a variety of conditions related to stress and had them answer a questionnaire prior to and following a flotation session. Participants reported decreases in muscle tension, pain and symptoms of their conditions after a single, 1-hour float, alongside an increase in feelings of relaxation and overall wellbeing.&nbsp;<br><br>Less is known about what effects sensory deprivation can have on the brain. To investigate, Feinstein's team had 48 people participate in either three 90-minute float sessions or three 90-minute periods of relaxing on a chair which reclined. Participants had their brains scanned using functional magnetic resonance imaging at the beginning and end of the trial. Float sessions uniquely decreased activity in the default mode network (DMN), a collection of brain regions commonly linked with depression. Feinstein says it is an exciting finding, because flotation tanks seem to offer a way of 'resetting' our nervous system to prevent it from getting out of balance.&nbsp;<br><br><strong>F </strong>Neurobiologist Tal Dotan Ben-Soussan is also an advocate of silence as a therapy. 'When we find ways to be quiet, we are not only quiet in our environment, but quiet in our inner selves,' she explains. 'This allows us to be more aware of what is happening around us and what the situation may require from us so we can provide [a more] adequate response.'&nbsp;<br><br>Not everyone will benefit from silence to the same extent, but Ben-Soussan says one characteristic is key: the person must need to want to engage in the experience. 'We see from animal models and human studies that volition and intentionality is important,' she says. 'When people do not want silence, it can be very distressing.'&nbsp;<br><br><strong>G&nbsp;</strong>Eric Pfeifer, a psychotherapy researcher, also concedes that some people may not benefit from silence, particularly those who are in a heightened state of stress. 'People in these states may not be able to relax or calm down in a silent condition,' he says. Professional guidance can be useful, he adds, allowing people to approach silence slowly so that they can gradually enjoy the benefits. And Pfeifer is convinced that silence is more attainable in everyday life than people think. First, complete silence isn't necessary. In a recent study, he found that participants reported more relaxation and less boredom when they sat quietly in an outdoor garden compared with a completely silent room. Second, Pfeifer believes we don't need a lot of silence to gain benefits. 'You don't need to spend hours in silence,' he says. 'It is likely better to have more frequency of silence for a few minutes at a time than a longer period of silence only once a week. Just finding those places in your daily life where you can find some silence … can make a big difference.'</p>
