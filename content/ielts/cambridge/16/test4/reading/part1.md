---
book_id: "f80bd8ea-f666-4d14-a9aa-bb59effd1c06"
book_number: 16
book_title: "Cambridge IELTS 16"
book_is_active: true
test_id: "f12c5c32-9412-4ad8-a0b0-3e181eaf3e39"
test_number: 4
test_title: "Cambridge IELTS 16 Test 4"
module_id: "8c6ebd1e-0e59-4589-89bf-950748e06a73"
module_type: "reading"
module_title: "Cambridge IELTS 16 Test 4 Reading"
duration_minutes: 60
module_sort_order: 2
section_id: "bfd512d4-ae6c-43f3-8200-f958548dd7dc"
section_number: 1
section_title: "Part 1"
section_instruction: "<p>You should spend about 20 minutes on&nbsp;Questions 1-13, which are based on Reading Passage 1 below.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>"
passage_title: null
section_sort_order: 1
module_raw_data_json: |
  {}
section_raw_data_json: |
  {
    "id": "320",
    "desc": "<p>You should spend about 20 minutes on&nbsp;Questions 1-13, which are based on Reading Passage 1 below.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>",
    "audio": "https://uimg.gximg.cn/ieltscb/exam/mp3/20230328/ab395407246edf05d2217b65702dd979.mp3",
    "pages": [
      {
        "desc": "<p><em>Label the diagrams below.</em></p><p><em>Choose <strong>ONE WORD ONLY</strong> from the passage for each answer.</em></p><p><em>Write your answers in boxes 1-6 on your answer sheet.</em>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;</p>",
        "type": "10",
        "title": "Questions 1 - 6",
        "content": "<figure class=\"image\"><img style=\"aspect-ratio:791/867;\" src=\"https://res.ytaxx.com/ielts/common/20260513/1778637309064.png\" width=\"791\" height=\"867\"></figure><p>1.&nbsp;&nbsp;<span data-question-id=\"1\">#####-1-#####</span>&nbsp;to direct the tunnelling</p><p>2. water runs into a&nbsp;<span data-question-id=\"2\">#####-2-#####</span>&nbsp;used by local people<br>&nbsp;</p><p>3. vertical shafts to remove earth and for&nbsp;<span data-question-id=\"3\">#####-3-#####</span>&nbsp;</p><p>4.&nbsp;<span data-question-id=\"4\">#####-4-#####</span>&nbsp;made of wood or stone</p><p>5.&nbsp;<span data-question-id=\"5\">#####-5-#####</span>&nbsp;attached to the plumb line</p><p>6. handholds and footholds used for&nbsp;<span data-question-id=\"6\">#####-6-#####</span>&nbsp;</p>",
        "realType": "11",
        "sections": [
          {
            "id": "1019",
            "res": "/exam/image/20230207/71f0b511626c3573173c7321b2042227.jpg",
            "desc": "",
            "type": "10",
            "title": "",
            "partId": 320,
            "options": [
              {
                "id": 4663,
                "title": "posts",
                "legend": 0,
                "sortOrder": 0
              },
              {
                "id": 4664,
                "title": "canal",
                "legend": 0,
                "sortOrder": 1
              },
              {
                "id": 4665,
                "title": "ventilation",
                "legend": 0,
                "sortOrder": 2
              },
              {
                "id": 4666,
                "title": "lid",
                "legend": 0,
                "sortOrder": 3
              },
              {
                "id": 4667,
                "title": "weight",
                "legend": 0,
                "sortOrder": 4
              },
              {
                "id": 4668,
                "title": "climbing",
                "legend": 0,
                "sortOrder": 5
              }
            ],
            "paperId": 36,
            "classify": "2",
            "realType": "11",
            "chapterId": "719",
            "questions": [
              {
                "id": "11723",
                "desc": "",
                "sort": "1",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4663",
                "sectionId": "1019",
                "questionNo": 1,
                "answerValue": "",
                "answerExplain": "定位词qanat method\n对应原文：第一段第二句：\nIn the early first millennium BCE, they introduced the qanat method of tunnel construction, \nwhich consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route,and then digging vertical shafts down into the ground at regular intervals. \n本题定位简单，在原文首先就看到了题目标题的qanat method，根据1题的题目（1）to direct the tunnelling，对应to ensure that tunnel kept to its route。根据图，空格中要填的是放置在山上的东西，应该是有复数的。回到原文，看到了which consisted of placing posts over a hill in a straight line, 就能找到答案，placing考动词含义，放置的意思。放置的是posts，对应图上所指，故答案为posts。\n定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal\n定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation\n定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。\n定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。\n定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。",
                "firstQuestion": true
              },
              {
                "id": "11724",
                "desc": "",
                "sort": "2",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4664",
                "sectionId": "1019",
                "questionNo": 2,
                "answerValue": "",
                "answerExplain": "定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal",
                "firstQuestion": false
              },
              {
                "id": "11725",
                "desc": "",
                "sort": "3",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4665",
                "sectionId": "1019",
                "questionNo": 3,
                "answerValue": "",
                "answerExplain": "定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation",
                "firstQuestion": false
              },
              {
                "id": "11726",
                "desc": "",
                "sort": "4",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4666",
                "sectionId": "1019",
                "questionNo": 4,
                "answerValue": "",
                "answerExplain": "定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。",
                "firstQuestion": false
              },
              {
                "id": "11727",
                "desc": "",
                "sort": "5",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4667",
                "sectionId": "1019",
                "questionNo": 5,
                "answerValue": "",
                "answerExplain": "定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。",
                "firstQuestion": false
              },
              {
                "id": "11728",
                "desc": "",
                "sort": "6",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4668",
                "sectionId": "1019",
                "questionNo": 6,
                "answerValue": "",
                "answerExplain": "定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。",
                "firstQuestion": false
              }
            ]
          }
        ],
        "questionGroupId": 604
      },
      {
        "desc": "<p><em>Do the following statements agree with the information given in Reading Passage 1? Choose&nbsp;</em></p><p><em><strong>TRUE</strong> if the statement agrees with the information&nbsp;</em></p><p><em><strong>FALSE</strong> if the statement contradicts the information</em></p><p><em><strong>NOT GIVEN</strong> if there is no information on this</em>&nbsp;&nbsp;</p>",
        "type": "7",
        "title": "Questions 7 - 10",
        "content": "<p>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</p>",
        "realType": "8",
        "sections": [
          {
            "id": "1020",
            "res": "",
            "desc": "",
            "type": "7",
            "title": "The counter-excavation method completely replaced the qanat method in the 6th century BCE.",
            "partId": 320,
            "options": [
              {
                "id": 4669,
                "title": "TRUE",
                "legend": 0,
                "sortOrder": 0
              },
              {
                "id": 4670,
                "title": "FALSE",
                "legend": 0,
                "sortOrder": 1
              },
              {
                "id": 4671,
                "title": "NOT GIVEN",
                "legend": 0,
                "sortOrder": 2
              }
            ],
            "paperId": 36,
            "classify": "2",
            "realType": "8",
            "chapterId": "719",
            "questions": [
              {
                "id": "11729",
                "desc": "",
                "sort": "7",
                "title": "The counter-excavation method completely replaced the qanat method in the 6th century BCE.",
                "answerId": "",
                "optionId": "",
                "optionIds": "4670",
                "sectionId": "1020",
                "questionNo": 7,
                "answerValue": "",
                "answerExplain": "定位词Counter-excavation method, completely replaced, in the 6th century BCE\n对应原文：第三段第一句：\nBy the 6th century BCE, a second method of tunnel construction appeared called the counter-excavation method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the qanat method was not a practical alternative. \n题目强调的是counter-excavation完全取代了qanat method在6世纪。回到原文定位是非常确定的，首先看到的是6th century BCE，然后就是第二个隧道修建的方法，名字叫做counter-excavation方法出现，需要确定的就是“是否有完全替代”，原文中给的是”当qanat method不是一个实用的选择的时候，被使用去穿过高山“。根据原文意思counter-excavation方法并没有完全取代qanat method，而是当qanat method不实用的时候就可以用。所以原文信息驳斥了题目意思，答案F。\n定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。\n定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。\n定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. ",
                "firstQuestion": false
              },
              {
                "id": "11730",
                "desc": "",
                "sort": "8",
                "title": " Only experienced builders were employed to construct a tunnel using the counter-excavation method.",
                "answerId": "",
                "optionId": "",
                "optionIds": "4671",
                "sectionId": "1020",
                "questionNo": 8,
                "answerValue": "",
                "answerExplain": "定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。",
                "firstQuestion": false
              },
              {
                "id": "11731",
                "desc": "",
                "sort": "9",
                "title": "The information about a problem that occurred during the construction of the Saldae aqueduct system was found in an ancient book.",
                "answerId": "",
                "optionId": "",
                "optionIds": "4670",
                "sectionId": "1020",
                "questionNo": 9,
                "answerValue": "",
                "answerExplain": "定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。",
                "firstQuestion": false
              },
              {
                "id": "11732",
                "desc": "",
                "sort": "10",
                "title": "The mistake made by the builders of the Saldae aqueduct system was that the two parts of the tunnel failed to meet.",
                "answerId": "",
                "optionId": "",
                "optionIds": "4669",
                "sectionId": "1020",
                "questionNo": 10,
                "answerValue": "",
                "answerExplain": "定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. ",
                "firstQuestion": false
              }
            ]
          }
        ],
        "questionGroupId": 605
      },
      {
        "desc": "<p><em>Answer the questions below.&nbsp;</em></p><p><em>Choose <strong>NO MORE THAN TWO WORDS</strong> from the passage for each answer&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</em></p>",
        "type": "10",
        "title": "Questions 11 - 13",
        "content": "<p><strong>11.</strong> What type of mineral were the Dolaucothi mines in Wales built to extract?</p><p>&nbsp;<span data-question-id=\"11\">#####-11-#####</span>&nbsp;</p><p>&nbsp;</p><p><strong>12.</strong> In addition to the patron, whose name might be carved onto a tunnel?</p><p>&nbsp;&nbsp;<span data-question-id=\"12\">#####-12-#####</span>&nbsp;</p><p>&nbsp;</p><p><strong>13.</strong> What part of Seleuceia Pieria was the Cevlik tunnel built to protect?</p><p>&nbsp;<span data-question-id=\"13\">#####-13-#####</span>&nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<br>&nbsp;</p>",
        "realType": "11",
        "sections": [
          {
            "id": "1021",
            "res": "",
            "desc": "",
            "type": "10",
            "title": "",
            "partId": 320,
            "options": [
              {
                "id": 4672,
                "title": "gold",
                "legend": 0,
                "sortOrder": 0
              },
              {
                "id": 4673,
                "title": "architect",
                "legend": 0,
                "sortOrder": 1
              },
              {
                "id": 4674,
                "title": "the architect's name",
                "legend": 0,
                "sortOrder": 2
              },
              {
                "id": 4675,
                "title": "the harbour",
                "legend": 0,
                "sortOrder": 3
              },
              {
                "id": 4676,
                "title": "harbor",
                "legend": 0,
                "sortOrder": 4
              },
              {
                "id": 4677,
                "title": "harbour",
                "legend": 0,
                "sortOrder": 5
              }
            ],
            "paperId": 36,
            "classify": "2",
            "realType": "11",
            "chapterId": "719",
            "questions": [
              {
                "id": "11733",
                "desc": "",
                "sort": "11",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4672",
                "sectionId": "1021",
                "questionNo": 11,
                "answerValue": "",
                "answerExplain": "定位词Mineral, Dolaucothi mines in Wales\n对应原文：第四段第五句：\nTraces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. \n题目的意思：Wales的Dolaucothi矿是被修建用来开采哪一种类型的矿？根据Dolaucothi和Wales定位，回到原文直接找到了at the Dolaucothi mines in Wales，句子里的traces of such tunnels used to mine gold，是用来开采金子的。所以答案确定为gold.\n定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.\n定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor",
                "firstQuestion": false
              },
              {
                "id": "11734",
                "desc": "",
                "sort": "12",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4673|4674",
                "sectionId": "1021",
                "questionNo": 12,
                "answerValue": "",
                "answerExplain": "定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.",
                "firstQuestion": false
              },
              {
                "id": "11735",
                "desc": "",
                "sort": "13",
                "title": "",
                "answerId": "",
                "optionId": "",
                "optionIds": "4675|4676|4677",
                "sectionId": "1021",
                "questionNo": 13,
                "answerValue": "",
                "answerExplain": "定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor",
                "firstQuestion": false
              }
            ]
          }
        ],
        "questionGroupId": 606
      }
    ],
    "title": "Part 1",
    "value": "",
    "mobile": "",
    "content": "<p style=\"text-align:center;\"><span style=\"font-size:22px;\"><strong>Roman tunnels</strong></span></p><p style=\"text-align:center;\"><i>The Romans, who once controlled areas of Europe, North Africa and Asia Minor, adopted the construction techniques of other civilizations to build tunnels in their territories</i></p><p style=\"text-align:center;\">&nbsp;</p><p style=\"text-align:left;\">The Persians, who lived in present-day Iran, were one of the first civilizations to build tunnels that provided a relia<span style=\"font-size:16px;\">ble supply of water to human settlements in dry areas. In the early first millennium BCE, they introduce</span>d the <i>qanat</i> method of tunnel construction, which consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route, and then digging vertical shafts down into the ground at regular intervals. Underground, workers removed the earth from between the ends of the shafts, creating a tunnel. The excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. Remarkably, some <i>qanats</i> built by the Persians 2,700 years ago are still in use today.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">They later passed on their knowledge to the Romans, who also used the <i>qanat</i> method to construct water-supply tunnels for agriculture. Roman <i>qanat</i> tunnels were constructed with vertical shafts dug at intervals of between 30 and 60 meters. The shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid. To ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft. Plumb lines were also used to measure the depth of the shaft and to determine the slope of the tunnel. The 5.6-kilometer-long Claudius tunnel, built in 41 CE to drain the Fucine Lake in central Italy, had shafts that were up to 122 meters deep, took 11 years to build and involved approximately 30,000 workers.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">By the 6th century BCE, a second method of tunnel construction appeared called the <i>counter-excavation</i> method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the <i>qanat</i> method was not a practical alternative. This method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. Adjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. They constantly checked the tunnel's advancing direction, for example, by looking back at the light that penetrated through the tunnel mouth, and made corrections whenever necessary. Large deviations could happen, and they could result in one end of the tunnel not being usable. An inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Algeria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">The Romans dug tunnels for their roads using the <i>counter-excavation</i> method, whenever they encountered obstacles such as hills or mountains that were too high for roads to pass over. An example is the 37-meter-long, 6-meter-high, Furlo Pass Tunnel built in Italy in 69-79 CE. Remarkably, a modern road still uses this tunnel today. Tunnels were also built for mineral extraction. Miners would locate a mineral vein and then pursue it with shafts and tunnels underground. Traces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. When the sole purpose of a tunnel was mineral extraction, construction required less planning, as the tunnel route was determined by the mineral vein.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">Roman tunnel projects were carefully planned and carried out. The length of time it took to construct a tunnel depended on the method being used and the type of rock being excavated. The <i>qanat</i> construction method was usually faster than the <i>counter-excavation</i> method as it was more straightforward. This was because the mountain could be excavated not only from the tunnel mouths but also from shafts. The type of rock could also influence construction times. When the rock was hard, the Romans employed a technique called fire quenching which consisted of heating the rock with fire, and then suddenly cooling it with cold water so that it would crack. Progress through hard rock could be very slow, and it was not uncommon for tunnels to take years, if not decades, to be built. Construction marks left on a Roman tunnel in Bologna show that the rate of advance through solid rock was 30 centimeters per day. In contrast, the rate of advance of the Claudius tunnel can be calculated at 1.4 meters per day. Most tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. For example, the 1.4-kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Seleuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81 CE.</p>",
    "paperId": "36",
    "classify": "2",
    "isSubmit": "1",
    "subjectId": 107,
    "writingList": [],
    "originalList": [],
    "analysisAudio": ""
  }
questions_json: |
  [
    {
      "id": "711d3d3a-10e6-48c4-ba26-fdace9743ee5",
      "section_id": "bfd512d4-ae6c-43f3-8200-f958548dd7dc",
      "question_number_start": 1,
      "question_number_end": 6,
      "question_type": "11",
      "prompt": "Questions 1 - 6",
      "instruction": "<p><em>Label the diagrams below.</em></p><p><em>Choose <strong>ONE WORD ONLY</strong> from the passage for each answer.</em></p><p><em>Write your answers in boxes 1-6 on your answer sheet.</em>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;</p>",
      "content": {
        "page_desc": "<p><em>Label the diagrams below.</em></p><p><em>Choose <strong>ONE WORD ONLY</strong> from the passage for each answer.</em></p><p><em>Write your answers in boxes 1-6 on your answer sheet.</em>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;</p>",
        "questions": [
          {
            "id": "11723",
            "desc": "",
            "sort": "1",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4663",
            "sectionId": "1019",
            "questionNo": 1,
            "answerValue": "",
            "answerExplain": "定位词qanat method\n对应原文：第一段第二句：\nIn the early first millennium BCE, they introduced the qanat method of tunnel construction, \nwhich consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route,and then digging vertical shafts down into the ground at regular intervals. \n本题定位简单，在原文首先就看到了题目标题的qanat method，根据1题的题目（1）to direct the tunnelling，对应to ensure that tunnel kept to its route。根据图，空格中要填的是放置在山上的东西，应该是有复数的。回到原文，看到了which consisted of placing posts over a hill in a straight line, 就能找到答案，placing考动词含义，放置的意思。放置的是posts，对应图上所指，故答案为posts。\n定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal\n定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation\n定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。\n定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。\n定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。",
            "firstQuestion": true
          },
          {
            "id": "11724",
            "desc": "",
            "sort": "2",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4664",
            "sectionId": "1019",
            "questionNo": 2,
            "answerValue": "",
            "answerExplain": "定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal",
            "firstQuestion": false
          },
          {
            "id": "11725",
            "desc": "",
            "sort": "3",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4665",
            "sectionId": "1019",
            "questionNo": 3,
            "answerValue": "",
            "answerExplain": "定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation",
            "firstQuestion": false
          },
          {
            "id": "11726",
            "desc": "",
            "sort": "4",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4666",
            "sectionId": "1019",
            "questionNo": 4,
            "answerValue": "",
            "answerExplain": "定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。",
            "firstQuestion": false
          },
          {
            "id": "11727",
            "desc": "",
            "sort": "5",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4667",
            "sectionId": "1019",
            "questionNo": 5,
            "answerValue": "",
            "answerExplain": "定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。",
            "firstQuestion": false
          },
          {
            "id": "11728",
            "desc": "",
            "sort": "6",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4668",
            "sectionId": "1019",
            "questionNo": 6,
            "answerValue": "",
            "answerExplain": "定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。",
            "firstQuestion": false
          }
        ],
        "page_title": "Questions 1 - 6",
        "part_title": "Part 1",
        "page_content": "<figure class=\"image\"><img style=\"aspect-ratio:791/867;\" src=\"https://res.ytaxx.com/ielts/common/20260513/1778637309064.png\" width=\"791\" height=\"867\"></figure><p>1.&nbsp;&nbsp;<span data-question-id=\"1\">#####-1-#####</span>&nbsp;to direct the tunnelling</p><p>2. water runs into a&nbsp;<span data-question-id=\"2\">#####-2-#####</span>&nbsp;used by local people<br>&nbsp;</p><p>3. vertical shafts to remove earth and for&nbsp;<span data-question-id=\"3\">#####-3-#####</span>&nbsp;</p><p>4.&nbsp;<span data-question-id=\"4\">#####-4-#####</span>&nbsp;made of wood or stone</p><p>5.&nbsp;<span data-question-id=\"5\">#####-5-#####</span>&nbsp;attached to the plumb line</p><p>6. handholds and footholds used for&nbsp;<span data-question-id=\"6\">#####-6-#####</span>&nbsp;</p>",
        "part_content": "<p style=\"text-align:center;\"><span style=\"font-size:22px;\"><strong>Roman tunnels</strong></span></p><p style=\"text-align:center;\"><i>The Romans, who once controlled areas of Europe, North Africa and Asia Minor, adopted the construction techniques of other civilizations to build tunnels in their territories</i></p><p style=\"text-align:center;\">&nbsp;</p><p style=\"text-align:left;\">The Persians, who lived in present-day Iran, were one of the first civilizations to build tunnels that provided a relia<span style=\"font-size:16px;\">ble supply of water to human settlements in dry areas. In the early first millennium BCE, they introduce</span>d the <i>qanat</i> method of tunnel construction, which consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route, and then digging vertical shafts down into the ground at regular intervals. Underground, workers removed the earth from between the ends of the shafts, creating a tunnel. The excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. Remarkably, some <i>qanats</i> built by the Persians 2,700 years ago are still in use today.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">They later passed on their knowledge to the Romans, who also used the <i>qanat</i> method to construct water-supply tunnels for agriculture. Roman <i>qanat</i> tunnels were constructed with vertical shafts dug at intervals of between 30 and 60 meters. The shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid. To ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft. Plumb lines were also used to measure the depth of the shaft and to determine the slope of the tunnel. The 5.6-kilometer-long Claudius tunnel, built in 41 CE to drain the Fucine Lake in central Italy, had shafts that were up to 122 meters deep, took 11 years to build and involved approximately 30,000 workers.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">By the 6th century BCE, a second method of tunnel construction appeared called the <i>counter-excavation</i> method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the <i>qanat</i> method was not a practical alternative. This method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. Adjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. They constantly checked the tunnel's advancing direction, for example, by looking back at the light that penetrated through the tunnel mouth, and made corrections whenever necessary. Large deviations could happen, and they could result in one end of the tunnel not being usable. An inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Algeria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">The Romans dug tunnels for their roads using the <i>counter-excavation</i> method, whenever they encountered obstacles such as hills or mountains that were too high for roads to pass over. An example is the 37-meter-long, 6-meter-high, Furlo Pass Tunnel built in Italy in 69-79 CE. Remarkably, a modern road still uses this tunnel today. Tunnels were also built for mineral extraction. Miners would locate a mineral vein and then pursue it with shafts and tunnels underground. Traces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. When the sole purpose of a tunnel was mineral extraction, construction required less planning, as the tunnel route was determined by the mineral vein.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">Roman tunnel projects were carefully planned and carried out. The length of time it took to construct a tunnel depended on the method being used and the type of rock being excavated. The <i>qanat</i> construction method was usually faster than the <i>counter-excavation</i> method as it was more straightforward. This was because the mountain could be excavated not only from the tunnel mouths but also from shafts. The type of rock could also influence construction times. When the rock was hard, the Romans employed a technique called fire quenching which consisted of heating the rock with fire, and then suddenly cooling it with cold water so that it would crack. Progress through hard rock could be very slow, and it was not uncommon for tunnels to take years, if not decades, to be built. Construction marks left on a Roman tunnel in Bologna show that the rate of advance through solid rock was 30 centimeters per day. In contrast, the rate of advance of the Claudius tunnel can be calculated at 1.4 meters per day. Most tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. For example, the 1.4-kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Seleuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81 CE.</p>",
        "section_desc": null,
        "writing_list": [],
        "section_title": null
      },
      "options": [],
      "sort_order": 1
    },
    {
      "id": "b93b2447-676b-4e9e-9557-c480245595c6",
      "section_id": "bfd512d4-ae6c-43f3-8200-f958548dd7dc",
      "question_number_start": 7,
      "question_number_end": 10,
      "question_type": "8",
      "prompt": "Questions 7 - 10",
      "instruction": "<p><em>Do the following statements agree with the information given in Reading Passage 1? Choose&nbsp;</em></p><p><em><strong>TRUE</strong> if the statement agrees with the information&nbsp;</em></p><p><em><strong>FALSE</strong> if the statement contradicts the information</em></p><p><em><strong>NOT GIVEN</strong> if there is no information on this</em>&nbsp;&nbsp;</p>",
      "content": {
        "page_desc": "<p><em>Do the following statements agree with the information given in Reading Passage 1? Choose&nbsp;</em></p><p><em><strong>TRUE</strong> if the statement agrees with the information&nbsp;</em></p><p><em><strong>FALSE</strong> if the statement contradicts the information</em></p><p><em><strong>NOT GIVEN</strong> if there is no information on this</em>&nbsp;&nbsp;</p>",
        "questions": [
          {
            "id": "11729",
            "desc": "",
            "sort": "7",
            "title": "The counter-excavation method completely replaced the qanat method in the 6th century BCE.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4670",
            "sectionId": "1020",
            "questionNo": 7,
            "answerValue": "",
            "answerExplain": "定位词Counter-excavation method, completely replaced, in the 6th century BCE\n对应原文：第三段第一句：\nBy the 6th century BCE, a second method of tunnel construction appeared called the counter-excavation method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the qanat method was not a practical alternative. \n题目强调的是counter-excavation完全取代了qanat method在6世纪。回到原文定位是非常确定的，首先看到的是6th century BCE，然后就是第二个隧道修建的方法，名字叫做counter-excavation方法出现，需要确定的就是“是否有完全替代”，原文中给的是”当qanat method不是一个实用的选择的时候，被使用去穿过高山“。根据原文意思counter-excavation方法并没有完全取代qanat method，而是当qanat method不实用的时候就可以用。所以原文信息驳斥了题目意思，答案F。\n定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。\n定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。\n定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. ",
            "firstQuestion": false
          },
          {
            "id": "11730",
            "desc": "",
            "sort": "8",
            "title": " Only experienced builders were employed to construct a tunnel using the counter-excavation method.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4671",
            "sectionId": "1020",
            "questionNo": 8,
            "answerValue": "",
            "answerExplain": "定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。",
            "firstQuestion": false
          },
          {
            "id": "11731",
            "desc": "",
            "sort": "9",
            "title": "The information about a problem that occurred during the construction of the Saldae aqueduct system was found in an ancient book.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4670",
            "sectionId": "1020",
            "questionNo": 9,
            "answerValue": "",
            "answerExplain": "定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。",
            "firstQuestion": false
          },
          {
            "id": "11732",
            "desc": "",
            "sort": "10",
            "title": "The mistake made by the builders of the Saldae aqueduct system was that the two parts of the tunnel failed to meet.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4669",
            "sectionId": "1020",
            "questionNo": 10,
            "answerValue": "",
            "answerExplain": "定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. ",
            "firstQuestion": false
          }
        ],
        "page_title": "Questions 7 - 10",
        "part_title": "Part 1",
        "page_content": "<p>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</p>",
        "part_content": "<p style=\"text-align:center;\"><span style=\"font-size:22px;\"><strong>Roman tunnels</strong></span></p><p style=\"text-align:center;\"><i>The Romans, who once controlled areas of Europe, North Africa and Asia Minor, adopted the construction techniques of other civilizations to build tunnels in their territories</i></p><p style=\"text-align:center;\">&nbsp;</p><p style=\"text-align:left;\">The Persians, who lived in present-day Iran, were one of the first civilizations to build tunnels that provided a relia<span style=\"font-size:16px;\">ble supply of water to human settlements in dry areas. In the early first millennium BCE, they introduce</span>d the <i>qanat</i> method of tunnel construction, which consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route, and then digging vertical shafts down into the ground at regular intervals. Underground, workers removed the earth from between the ends of the shafts, creating a tunnel. The excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. Remarkably, some <i>qanats</i> built by the Persians 2,700 years ago are still in use today.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">They later passed on their knowledge to the Romans, who also used the <i>qanat</i> method to construct water-supply tunnels for agriculture. Roman <i>qanat</i> tunnels were constructed with vertical shafts dug at intervals of between 30 and 60 meters. The shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid. To ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft. Plumb lines were also used to measure the depth of the shaft and to determine the slope of the tunnel. The 5.6-kilometer-long Claudius tunnel, built in 41 CE to drain the Fucine Lake in central Italy, had shafts that were up to 122 meters deep, took 11 years to build and involved approximately 30,000 workers.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">By the 6th century BCE, a second method of tunnel construction appeared called the <i>counter-excavation</i> method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the <i>qanat</i> method was not a practical alternative. This method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. Adjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. They constantly checked the tunnel's advancing direction, for example, by looking back at the light that penetrated through the tunnel mouth, and made corrections whenever necessary. Large deviations could happen, and they could result in one end of the tunnel not being usable. An inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Algeria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">The Romans dug tunnels for their roads using the <i>counter-excavation</i> method, whenever they encountered obstacles such as hills or mountains that were too high for roads to pass over. An example is the 37-meter-long, 6-meter-high, Furlo Pass Tunnel built in Italy in 69-79 CE. Remarkably, a modern road still uses this tunnel today. Tunnels were also built for mineral extraction. Miners would locate a mineral vein and then pursue it with shafts and tunnels underground. Traces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. When the sole purpose of a tunnel was mineral extraction, construction required less planning, as the tunnel route was determined by the mineral vein.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">Roman tunnel projects were carefully planned and carried out. The length of time it took to construct a tunnel depended on the method being used and the type of rock being excavated. The <i>qanat</i> construction method was usually faster than the <i>counter-excavation</i> method as it was more straightforward. This was because the mountain could be excavated not only from the tunnel mouths but also from shafts. The type of rock could also influence construction times. When the rock was hard, the Romans employed a technique called fire quenching which consisted of heating the rock with fire, and then suddenly cooling it with cold water so that it would crack. Progress through hard rock could be very slow, and it was not uncommon for tunnels to take years, if not decades, to be built. Construction marks left on a Roman tunnel in Bologna show that the rate of advance through solid rock was 30 centimeters per day. In contrast, the rate of advance of the Claudius tunnel can be calculated at 1.4 meters per day. Most tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. For example, the 1.4-kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Seleuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81 CE.</p>",
        "section_desc": null,
        "writing_list": [],
        "section_title": "The counter-excavation method completely replaced the qanat method in the 6th century BCE."
      },
      "options": [
        {
          "id": 4669,
          "title": "TRUE",
          "legend": 0,
          "sortOrder": 0
        },
        {
          "id": 4670,
          "title": "FALSE",
          "legend": 0,
          "sortOrder": 1
        },
        {
          "id": 4671,
          "title": "NOT GIVEN",
          "legend": 0,
          "sortOrder": 2
        }
      ],
      "sort_order": 2
    },
    {
      "id": "5787ff5f-ffef-4454-aaab-1015153477d1",
      "section_id": "bfd512d4-ae6c-43f3-8200-f958548dd7dc",
      "question_number_start": 11,
      "question_number_end": 13,
      "question_type": "11",
      "prompt": "Questions 11 - 13",
      "instruction": "<p><em>Answer the questions below.&nbsp;</em></p><p><em>Choose <strong>NO MORE THAN TWO WORDS</strong> from the passage for each answer&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</em></p>",
      "content": {
        "page_desc": "<p><em>Answer the questions below.&nbsp;</em></p><p><em>Choose <strong>NO MORE THAN TWO WORDS</strong> from the passage for each answer&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</em></p>",
        "questions": [
          {
            "id": "11733",
            "desc": "",
            "sort": "11",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4672",
            "sectionId": "1021",
            "questionNo": 11,
            "answerValue": "",
            "answerExplain": "定位词Mineral, Dolaucothi mines in Wales\n对应原文：第四段第五句：\nTraces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. \n题目的意思：Wales的Dolaucothi矿是被修建用来开采哪一种类型的矿？根据Dolaucothi和Wales定位，回到原文直接找到了at the Dolaucothi mines in Wales，句子里的traces of such tunnels used to mine gold，是用来开采金子的。所以答案确定为gold.\n定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.\n定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor",
            "firstQuestion": false
          },
          {
            "id": "11734",
            "desc": "",
            "sort": "12",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4673|4674",
            "sectionId": "1021",
            "questionNo": 12,
            "answerValue": "",
            "answerExplain": "定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.",
            "firstQuestion": false
          },
          {
            "id": "11735",
            "desc": "",
            "sort": "13",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4675|4676|4677",
            "sectionId": "1021",
            "questionNo": 13,
            "answerValue": "",
            "answerExplain": "定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor",
            "firstQuestion": false
          }
        ],
        "page_title": "Questions 11 - 13",
        "part_title": "Part 1",
        "page_content": "<p><strong>11.</strong> What type of mineral were the Dolaucothi mines in Wales built to extract?</p><p>&nbsp;<span data-question-id=\"11\">#####-11-#####</span>&nbsp;</p><p>&nbsp;</p><p><strong>12.</strong> In addition to the patron, whose name might be carved onto a tunnel?</p><p>&nbsp;&nbsp;<span data-question-id=\"12\">#####-12-#####</span>&nbsp;</p><p>&nbsp;</p><p><strong>13.</strong> What part of Seleuceia Pieria was the Cevlik tunnel built to protect?</p><p>&nbsp;<span data-question-id=\"13\">#####-13-#####</span>&nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<br>&nbsp;</p>",
        "part_content": "<p style=\"text-align:center;\"><span style=\"font-size:22px;\"><strong>Roman tunnels</strong></span></p><p style=\"text-align:center;\"><i>The Romans, who once controlled areas of Europe, North Africa and Asia Minor, adopted the construction techniques of other civilizations to build tunnels in their territories</i></p><p style=\"text-align:center;\">&nbsp;</p><p style=\"text-align:left;\">The Persians, who lived in present-day Iran, were one of the first civilizations to build tunnels that provided a relia<span style=\"font-size:16px;\">ble supply of water to human settlements in dry areas. In the early first millennium BCE, they introduce</span>d the <i>qanat</i> method of tunnel construction, which consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route, and then digging vertical shafts down into the ground at regular intervals. Underground, workers removed the earth from between the ends of the shafts, creating a tunnel. The excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. Remarkably, some <i>qanats</i> built by the Persians 2,700 years ago are still in use today.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">They later passed on their knowledge to the Romans, who also used the <i>qanat</i> method to construct water-supply tunnels for agriculture. Roman <i>qanat</i> tunnels were constructed with vertical shafts dug at intervals of between 30 and 60 meters. The shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid. To ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft. Plumb lines were also used to measure the depth of the shaft and to determine the slope of the tunnel. The 5.6-kilometer-long Claudius tunnel, built in 41 CE to drain the Fucine Lake in central Italy, had shafts that were up to 122 meters deep, took 11 years to build and involved approximately 30,000 workers.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">By the 6th century BCE, a second method of tunnel construction appeared called the <i>counter-excavation</i> method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the <i>qanat</i> method was not a practical alternative. This method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. Adjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. They constantly checked the tunnel's advancing direction, for example, by looking back at the light that penetrated through the tunnel mouth, and made corrections whenever necessary. Large deviations could happen, and they could result in one end of the tunnel not being usable. An inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Algeria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">The Romans dug tunnels for their roads using the <i>counter-excavation</i> method, whenever they encountered obstacles such as hills or mountains that were too high for roads to pass over. An example is the 37-meter-long, 6-meter-high, Furlo Pass Tunnel built in Italy in 69-79 CE. Remarkably, a modern road still uses this tunnel today. Tunnels were also built for mineral extraction. Miners would locate a mineral vein and then pursue it with shafts and tunnels underground. Traces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. When the sole purpose of a tunnel was mineral extraction, construction required less planning, as the tunnel route was determined by the mineral vein.</p><p style=\"text-align:left;\">&nbsp;</p><p style=\"text-align:left;\">Roman tunnel projects were carefully planned and carried out. The length of time it took to construct a tunnel depended on the method being used and the type of rock being excavated. The <i>qanat</i> construction method was usually faster than the <i>counter-excavation</i> method as it was more straightforward. This was because the mountain could be excavated not only from the tunnel mouths but also from shafts. The type of rock could also influence construction times. When the rock was hard, the Romans employed a technique called fire quenching which consisted of heating the rock with fire, and then suddenly cooling it with cold water so that it would crack. Progress through hard rock could be very slow, and it was not uncommon for tunnels to take years, if not decades, to be built. Construction marks left on a Roman tunnel in Bologna show that the rate of advance through solid rock was 30 centimeters per day. In contrast, the rate of advance of the Claudius tunnel can be calculated at 1.4 meters per day. Most tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. For example, the 1.4-kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Seleuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81 CE.</p>",
        "section_desc": null,
        "writing_list": [],
        "section_title": null
      },
      "options": [],
      "sort_order": 3
    }
  ]
answers_json: |
  [
    {
      "id": "94a5a1f5-8cb8-431e-a0fc-d4e091fc372f",
      "question_id": "711d3d3a-10e6-48c4-ba26-fdace9743ee5",
      "answer_data": {
        "answers": [
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4663",
            "question_no": 1,
            "answer_value": null,
            "answer_explain": "定位词qanat method\n对应原文：第一段第二句：\nIn the early first millennium BCE, they introduced the qanat method of tunnel construction, \nwhich consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route,and then digging vertical shafts down into the ground at regular intervals. \n本题定位简单，在原文首先就看到了题目标题的qanat method，根据1题的题目（1）to direct the tunnelling，对应to ensure that tunnel kept to its route。根据图，空格中要填的是放置在山上的东西，应该是有复数的。回到原文，看到了which consisted of placing posts over a hill in a straight line, 就能找到答案，placing考动词含义，放置的意思。放置的是posts，对应图上所指，故答案为posts。\n定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal\n定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation\n定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。\n定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。\n定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。"
          },
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4664",
            "question_no": 2,
            "answer_value": null,
            "answer_explain": "定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal"
          },
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4665",
            "question_no": 3,
            "answer_value": null,
            "answer_explain": "定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation"
          },
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4666",
            "question_no": 4,
            "answer_value": null,
            "answer_explain": "定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。"
          },
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4667",
            "question_no": 5,
            "answer_value": null,
            "answer_explain": "定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。"
          },
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4668",
            "question_no": 6,
            "answer_value": null,
            "answer_explain": "定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。"
          }
        ],
        "options": [
          {
            "id": 4663,
            "title": "posts",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 4664,
            "title": "canal",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 4665,
            "title": "ventilation",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 4666,
            "title": "lid",
            "legend": 0,
            "sortOrder": 3
          },
          {
            "id": 4667,
            "title": "weight",
            "legend": 0,
            "sortOrder": 4
          },
          {
            "id": 4668,
            "title": "climbing",
            "legend": 0,
            "sortOrder": 5
          }
        ]
      },
      "explanation": "定位词qanat method\n对应原文：第一段第二句：\nIn the early first millennium BCE, they introduced the qanat method of tunnel construction, \nwhich consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route,and then digging vertical shafts down into the ground at regular intervals. \n本题定位简单，在原文首先就看到了题目标题的qanat method，根据1题的题目（1）to direct the tunnelling，对应to ensure that tunnel kept to its route。根据图，空格中要填的是放置在山上的东西，应该是有复数的。回到原文，看到了which consisted of placing posts over a hill in a straight line, 就能找到答案，placing考动词含义，放置的意思。放置的是posts，对应图上所指，故答案为posts。\n定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal\n定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation\n定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。\n定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。\n定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。\n\n定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal\n\n定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation\n\n定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。\n\n定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。\n\n定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。",
      "raw_data": {
        "id": "1019",
        "res": "/exam/image/20230207/71f0b511626c3573173c7321b2042227.jpg",
        "desc": "",
        "type": "10",
        "title": "",
        "partId": 320,
        "options": [
          {
            "id": 4663,
            "title": "posts",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 4664,
            "title": "canal",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 4665,
            "title": "ventilation",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 4666,
            "title": "lid",
            "legend": 0,
            "sortOrder": 3
          },
          {
            "id": 4667,
            "title": "weight",
            "legend": 0,
            "sortOrder": 4
          },
          {
            "id": 4668,
            "title": "climbing",
            "legend": 0,
            "sortOrder": 5
          }
        ],
        "paperId": 36,
        "classify": "2",
        "realType": "11",
        "chapterId": "719",
        "questions": [
          {
            "id": "11723",
            "desc": "",
            "sort": "1",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4663",
            "sectionId": "1019",
            "questionNo": 1,
            "answerValue": "",
            "answerExplain": "定位词qanat method\n对应原文：第一段第二句：\nIn the early first millennium BCE, they introduced the qanat method of tunnel construction, \nwhich consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route,and then digging vertical shafts down into the ground at regular intervals. \n本题定位简单，在原文首先就看到了题目标题的qanat method，根据1题的题目（1）to direct the tunnelling，对应to ensure that tunnel kept to its route。根据图，空格中要填的是放置在山上的东西，应该是有复数的。回到原文，看到了which consisted of placing posts over a hill in a straight line, 就能找到答案，placing考动词含义，放置的意思。放置的是posts，对应图上所指，故答案为posts。\n定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal\n定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation\n定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。\n定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。\n定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。",
            "firstQuestion": true
          },
          {
            "id": "11724",
            "desc": "",
            "sort": "2",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4664",
            "sectionId": "1019",
            "questionNo": 2,
            "answerValue": "",
            "answerExplain": "定位词water, local people \n对应原文：第一段第四句：Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. \n根据题目意思可以确定，考察的是水进入一个_____，被当地人使用。根据water定位到第四句，答案要的是水进入一个什么地方，对应原文中down towards a canal，故答案为canal。后面supplied water for human use对应题干中的used by local people。本题答案canal",
            "firstQuestion": false
          },
          {
            "id": "11725",
            "desc": "",
            "sort": "3",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4665",
            "sectionId": "1019",
            "questionNo": 3,
            "answerValue": "",
            "answerExplain": "定位词Vertical shafts， remove earth\n对应原文：第一段第三句：\nThe excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. \n题干意思vertical shafts去移动土地和为了_______，考察的是并列关系，在原文中找到remove earth就可以确定答案。回到原文the excavated soil was taken up to the surface using the shafts，shaft是原词，题目中的remove earth对应原文中是excavated soil ，土地被带到了表面。并列关系在原文中是由also给出的，所以答案直接确定是provided ventilation，提供通风，意思就是为了通风，空格中填一个名词，故答案为ventilation",
            "firstQuestion": false
          },
          {
            "id": "11726",
            "desc": "",
            "sort": "4",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4666",
            "sectionId": "1019",
            "questionNo": 4,
            "answerValue": "",
            "answerExplain": "定位词wood or stone\n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n根据题目的信息，made of wood or stone可以得知4题是由石头或者是木头做成的，根据图的表达，可以得知4题是在shaft的最上方。回到原文看到were covered with a wooden or stone lid。此处wooden和stone为形容词，修饰lid，故得知lid是wood或stone做的，故答案为lid（盖子）。",
            "firstQuestion": false
          },
          {
            "id": "11727",
            "desc": "",
            "sort": "5",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4667",
            "sectionId": "1019",
            "questionNo": 5,
            "answerValue": "",
            "answerExplain": "定位词plumb line\n对应原文：第二段第四句：\nTo ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.\n根据题目的意思，是_________附着在了一个铅垂线上。所以是要找到铅垂线最后的东西的答案。回到原文的hung a plumb line from a rod出现了Plumb line的原词，from a rod也就是题目中的从rod上悬挂下来。根据图示，5题这个物体是挂在plump line上的，原文中made sure the weight at the end of it，因此，答案是weight。（此处weight为名词，表示’砝码‘的意思。",
            "firstQuestion": false
          },
          {
            "id": "11728",
            "desc": "",
            "sort": "6",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4668",
            "sectionId": "1019",
            "questionNo": 6,
            "answerValue": "",
            "answerExplain": "定位词Handholds, footholds \n对应原文：第二段第三句：\nThe shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.\n题目中的handholds and footholds used for _______，定位给的非常明确，回到原文中看到了handholds and footholds的原词，原文中给的是去帮助那些人爬进去和爬出来。所以答案确定是used for (climbing），为了爬，故答案为climbing。",
            "firstQuestion": false
          }
        ]
      }
    },
    {
      "id": "1d81dc94-b34a-4d52-860d-36474e7ab53b",
      "question_id": "b93b2447-676b-4e9e-9557-c480245595c6",
      "answer_data": {
        "answers": [
          {
            "title": "The counter-excavation method completely replaced the qanat method in the 6th century BCE.",
            "answer_id": null,
            "option_ids": "4670",
            "question_no": 7,
            "answer_value": null,
            "answer_explain": "定位词Counter-excavation method, completely replaced, in the 6th century BCE\n对应原文：第三段第一句：\nBy the 6th century BCE, a second method of tunnel construction appeared called the counter-excavation method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the qanat method was not a practical alternative. \n题目强调的是counter-excavation完全取代了qanat method在6世纪。回到原文定位是非常确定的，首先看到的是6th century BCE，然后就是第二个隧道修建的方法，名字叫做counter-excavation方法出现，需要确定的就是“是否有完全替代”，原文中给的是”当qanat method不是一个实用的选择的时候，被使用去穿过高山“。根据原文意思counter-excavation方法并没有完全取代qanat method，而是当qanat method不实用的时候就可以用。所以原文信息驳斥了题目意思，答案F。\n定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。\n定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。\n定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. "
          },
          {
            "title": " Only experienced builders were employed to construct a tunnel using the counter-excavation method.",
            "answer_id": null,
            "option_ids": "4671",
            "question_no": 8,
            "answer_value": null,
            "answer_explain": "定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。"
          },
          {
            "title": "The information about a problem that occurred during the construction of the Saldae aqueduct system was found in an ancient book.",
            "answer_id": null,
            "option_ids": "4670",
            "question_no": 9,
            "answer_value": null,
            "answer_explain": "定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。"
          },
          {
            "title": "The mistake made by the builders of the Saldae aqueduct system was that the two parts of the tunnel failed to meet.",
            "answer_id": null,
            "option_ids": "4669",
            "question_no": 10,
            "answer_value": null,
            "answer_explain": "定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. "
          }
        ],
        "options": [
          {
            "id": 4669,
            "title": "TRUE",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 4670,
            "title": "FALSE",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 4671,
            "title": "NOT GIVEN",
            "legend": 0,
            "sortOrder": 2
          }
        ]
      },
      "explanation": "定位词Counter-excavation method, completely replaced, in the 6th century BCE\n对应原文：第三段第一句：\nBy the 6th century BCE, a second method of tunnel construction appeared called the counter-excavation method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the qanat method was not a practical alternative. \n题目强调的是counter-excavation完全取代了qanat method在6世纪。回到原文定位是非常确定的，首先看到的是6th century BCE，然后就是第二个隧道修建的方法，名字叫做counter-excavation方法出现，需要确定的就是“是否有完全替代”，原文中给的是”当qanat method不是一个实用的选择的时候，被使用去穿过高山“。根据原文意思counter-excavation方法并没有完全取代qanat method，而是当qanat method不实用的时候就可以用。所以原文信息驳斥了题目意思，答案F。\n定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。\n定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。\n定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. \n\n定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。\n\n定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。\n\n定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. ",
      "raw_data": {
        "id": "1020",
        "res": "",
        "desc": "",
        "type": "7",
        "title": "The counter-excavation method completely replaced the qanat method in the 6th century BCE.",
        "partId": 320,
        "options": [
          {
            "id": 4669,
            "title": "TRUE",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 4670,
            "title": "FALSE",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 4671,
            "title": "NOT GIVEN",
            "legend": 0,
            "sortOrder": 2
          }
        ],
        "paperId": 36,
        "classify": "2",
        "realType": "8",
        "chapterId": "719",
        "questions": [
          {
            "id": "11729",
            "desc": "",
            "sort": "7",
            "title": "The counter-excavation method completely replaced the qanat method in the 6th century BCE.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4670",
            "sectionId": "1020",
            "questionNo": 7,
            "answerValue": "",
            "answerExplain": "定位词Counter-excavation method, completely replaced, in the 6th century BCE\n对应原文：第三段第一句：\nBy the 6th century BCE, a second method of tunnel construction appeared called the counter-excavation method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the qanat method was not a practical alternative. \n题目强调的是counter-excavation完全取代了qanat method在6世纪。回到原文定位是非常确定的，首先看到的是6th century BCE，然后就是第二个隧道修建的方法，名字叫做counter-excavation方法出现，需要确定的就是“是否有完全替代”，原文中给的是”当qanat method不是一个实用的选择的时候，被使用去穿过高山“。根据原文意思counter-excavation方法并没有完全取代qanat method，而是当qanat method不实用的时候就可以用。所以原文信息驳斥了题目意思，答案F。\n定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。\n定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。\n定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. ",
            "firstQuestion": false
          },
          {
            "id": "11730",
            "desc": "",
            "sort": "8",
            "title": " Only experienced builders were employed to construct a tunnel using the counter-excavation method.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4671",
            "sectionId": "1020",
            "questionNo": 8,
            "answerValue": "",
            "answerExplain": "定位词Only experienced builders \n对应原文：第三段第三句：\nThis method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. \n第三段第四句：\nAdjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. \n题目的意思是：只有有经验的修建者被雇佣去修建隧道，使用counter-excavation方法。回到原文中，我们看到的是this method required，这个方法要求，但后文给的是更高的计划和更先进的调查，数学和几何学知识。没有说有经验的修建者。那第四句话出现了builders，但意思强调的是隧道方向的调整+需要被做+当修建者遇到了+地质的问题或者是当偏离了设定的道路的时候。并没有说只有有经验的修建者才能用counter excavation方法修建隧道。所以答案确定是Not Given。在T/F/NG题中，NG是一个比较容易出现错误的点。考生需要细细体会题目中的“Only”是在原文中没有出现的信息。答案为NG。",
            "firstQuestion": false
          },
          {
            "id": "11731",
            "desc": "",
            "sort": "9",
            "title": "The information about a problem that occurred during the construction of the Saldae aqueduct system was found in an ancient book.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4670",
            "sectionId": "1020",
            "questionNo": 9,
            "answerValue": "",
            "answerExplain": "定位词Problem, found in an ancient book\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.\n题目的意思是：关于一个问题的信息被发现在一个古老的书里，问题是发生在Saldae aqueduct系统的休闲的过程中的问题。回到原文，Saldae aqueduct system的定位是非常确定的。原文定位词所在的这句话写的是an inscription written on the side of a 428-meter tunnel，所以是写在一个428米长的隧道的一侧的inscription（题词，铭文），描述了两个团队是怎样错过的。答案在这里已经确定了，因为并不是题目中的found in an ancient book，发现在一个古老的书上，原文说的很明显是在隧道的一层上雕刻的。答案F。",
            "firstQuestion": false
          },
          {
            "id": "11732",
            "desc": "",
            "sort": "10",
            "title": "The mistake made by the builders of the Saldae aqueduct system was that the two parts of the tunnel failed to meet.",
            "answerId": "",
            "optionId": "",
            "optionIds": "4669",
            "sectionId": "1020",
            "questionNo": 10,
            "answerValue": "",
            "answerExplain": "定位词Mistake, two parts of the tunnel failed to meet\n对应原文：第三段第七句：\nAn inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Aleria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error. \n题目的意思是：Saldae aqueduct系统的修建者犯的错误是隧道的两边没有相遇。回到原文中，Saldae aqueduct系统和9题的定位词是一样的。The two teams of builders也是原词，两组修建者。两组修建者missed each other in the mountain在山里错过了彼此，和之后的a lateral link between both corridors corrected the initial error,意为“在两个通道之间的一个边侧连接修正了最初的错误” 因此，两个团队修建的隧道是没有汇合的，对应题干中failed to meet，题干与原文一致，答案为True. ",
            "firstQuestion": false
          }
        ]
      }
    },
    {
      "id": "095718e2-61e9-4b00-9fa6-5bb497529375",
      "question_id": "5787ff5f-ffef-4454-aaab-1015153477d1",
      "answer_data": {
        "answers": [
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4672",
            "question_no": 11,
            "answer_value": null,
            "answer_explain": "定位词Mineral, Dolaucothi mines in Wales\n对应原文：第四段第五句：\nTraces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. \n题目的意思：Wales的Dolaucothi矿是被修建用来开采哪一种类型的矿？根据Dolaucothi和Wales定位，回到原文直接找到了at the Dolaucothi mines in Wales，句子里的traces of such tunnels used to mine gold，是用来开采金子的。所以答案确定为gold.\n定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.\n定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor"
          },
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4673|4674",
            "question_no": 12,
            "answer_value": null,
            "answer_explain": "定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect."
          },
          {
            "title": null,
            "answer_id": null,
            "option_ids": "4675|4676|4677",
            "question_no": 13,
            "answer_value": null,
            "answer_explain": "定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor"
          }
        ],
        "options": [
          {
            "id": 4672,
            "title": "gold",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 4673,
            "title": "architect",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 4674,
            "title": "the architect's name",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 4675,
            "title": "the harbour",
            "legend": 0,
            "sortOrder": 3
          },
          {
            "id": 4676,
            "title": "harbor",
            "legend": 0,
            "sortOrder": 4
          },
          {
            "id": 4677,
            "title": "harbour",
            "legend": 0,
            "sortOrder": 5
          }
        ]
      },
      "explanation": "定位词Mineral, Dolaucothi mines in Wales\n对应原文：第四段第五句：\nTraces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. \n题目的意思：Wales的Dolaucothi矿是被修建用来开采哪一种类型的矿？根据Dolaucothi和Wales定位，回到原文直接找到了at the Dolaucothi mines in Wales，句子里的traces of such tunnels used to mine gold，是用来开采金子的。所以答案确定为gold.\n定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.\n定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor\n\n定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.\n\n定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor",
      "raw_data": {
        "id": "1021",
        "res": "",
        "desc": "",
        "type": "10",
        "title": "",
        "partId": 320,
        "options": [
          {
            "id": 4672,
            "title": "gold",
            "legend": 0,
            "sortOrder": 0
          },
          {
            "id": 4673,
            "title": "architect",
            "legend": 0,
            "sortOrder": 1
          },
          {
            "id": 4674,
            "title": "the architect's name",
            "legend": 0,
            "sortOrder": 2
          },
          {
            "id": 4675,
            "title": "the harbour",
            "legend": 0,
            "sortOrder": 3
          },
          {
            "id": 4676,
            "title": "harbor",
            "legend": 0,
            "sortOrder": 4
          },
          {
            "id": 4677,
            "title": "harbour",
            "legend": 0,
            "sortOrder": 5
          }
        ],
        "paperId": 36,
        "classify": "2",
        "realType": "11",
        "chapterId": "719",
        "questions": [
          {
            "id": "11733",
            "desc": "",
            "sort": "11",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4672",
            "sectionId": "1021",
            "questionNo": 11,
            "answerValue": "",
            "answerExplain": "定位词Mineral, Dolaucothi mines in Wales\n对应原文：第四段第五句：\nTraces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. \n题目的意思：Wales的Dolaucothi矿是被修建用来开采哪一种类型的矿？根据Dolaucothi和Wales定位，回到原文直接找到了at the Dolaucothi mines in Wales，句子里的traces of such tunnels used to mine gold，是用来开采金子的。所以答案确定为gold.\n定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.\n定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor",
            "firstQuestion": false
          },
          {
            "id": "11734",
            "desc": "",
            "sort": "12",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4673|4674",
            "sectionId": "1021",
            "questionNo": 12,
            "answerValue": "",
            "answerExplain": "定位词Patron, name\n对应原文：第五段第九句：\nMost tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. \n题目的意思：除了Patron，谁的名字有可能被雕刻在隧道上？原文中直接出现了showing the names of patrons定位，原文中的and替换了题目中的in addition to，表示并列关系，所以答案为并列的后者architect.",
            "firstQuestion": false
          },
          {
            "id": "11735",
            "desc": "",
            "sort": "13",
            "title": "",
            "answerId": "",
            "optionId": "",
            "optionIds": "4675|4676|4677",
            "sectionId": "1021",
            "questionNo": 13,
            "answerValue": "",
            "answerExplain": "定位词Seleuceia Pieria, Cevlik tunnel, protect\n对应原文：第五段第十句：\nFor example, the 1.4 kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Selecuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81CE.\n题目的意思：Seleuceia Pieria的哪一个部分是Cevlik隧道被修建去保护的。回到原文定位给的很好，Cevlik隧道是原词，被修剪是为了divert the floodwater threatening the harbor of the ancient city of SP，城市也是原词出现的。原文中的洪水威胁了古老的SP城市的海港，直接给出了意思是要保护“海港”，答案harbor确定。注意积累单词divert the floodwater, 转移了洪水+威胁了港湾。所以是为了保护港湾。答案harbor",
            "firstQuestion": false
          }
        ]
      }
    }
  ]
assets_json: |
  [
    {
      "id": "143cf54c-bfc0-4dc2-809f-86be3b4f292a",
      "book_id": "f80bd8ea-f666-4d14-a9aa-bb59effd1c06",
      "test_id": "f12c5c32-9412-4ad8-a0b0-3e181eaf3e39",
      "module_id": "8c6ebd1e-0e59-4589-89bf-950748e06a73",
      "section_id": null,
      "question_id": null,
      "asset_type": "json",
      "bucket": "ielts",
      "storage_path": "16/test-4/reading/raw.json",
      "public_url": "https://xnmlzimdpawdqikgbpvw.supabase.co/storage/v1/object/public/ielts/16/test-4/reading/raw.json",
      "mime_type": "application/json",
      "duration_seconds": null,
      "metadata": {
        "classify": 2,
        "source_url": "https://www.winielts.com/api/paper/exam/queryByIdAndClassify?id=36&classify=2&type=1",
        "source_paper_id": 36
      }
    },
    {
      "id": "6ee89948-af0c-4cef-a103-59b3d9627ccc",
      "book_id": "f80bd8ea-f666-4d14-a9aa-bb59effd1c06",
      "test_id": "f12c5c32-9412-4ad8-a0b0-3e181eaf3e39",
      "module_id": "8c6ebd1e-0e59-4589-89bf-950748e06a73",
      "section_id": null,
      "question_id": null,
      "asset_type": "audio",
      "bucket": "ielts",
      "storage_path": "16/test-4/reading/audio/ab395407246edf05d2217b65702dd979.mp3",
      "public_url": "https://xnmlzimdpawdqikgbpvw.supabase.co/storage/v1/object/public/ielts/16/test-4/reading/audio/ab395407246edf05d2217b65702dd979.mp3",
      "mime_type": "audio/mpeg",
      "duration_seconds": null,
      "metadata": {
        "bytes": 29120520,
        "error": null,
        "retried": true,
        "classify": 2,
        "source_url": "https://uimg.gximg.cn/ieltscb/exam/mp3/20230328/ab395407246edf05d2217b65702dd979.mp3",
        "source_paper_id": 36
      }
    },
    {
      "id": "096ae819-a182-4cf9-a802-e875e6cee419",
      "book_id": "f80bd8ea-f666-4d14-a9aa-bb59effd1c06",
      "test_id": "f12c5c32-9412-4ad8-a0b0-3e181eaf3e39",
      "module_id": "8c6ebd1e-0e59-4589-89bf-950748e06a73",
      "section_id": null,
      "question_id": null,
      "asset_type": "image",
      "bucket": "ielts",
      "storage_path": "16/test-4/reading/image/1778637309064.png",
      "public_url": "https://xnmlzimdpawdqikgbpvw.supabase.co/storage/v1/object/public/ielts/16/test-4/reading/image/1778637309064.png",
      "mime_type": "image/png",
      "duration_seconds": null,
      "metadata": {
        "bytes": 427573,
        "error": null,
        "classify": 2,
        "source_url": "https://res.ytaxx.com/ielts/common/20260513/1778637309064.png",
        "source_paper_id": 36
      }
    }
  ]
passage_translation_json: |
  [
    {
      "id": "s1",
      "en": "Roman tunnels",
      "zh": "罗马隧道"
    },
    {
      "id": "s2",
      "en": "The Romans, who once controlled areas of Europe, North Africa and Asia Minor, adopted the construction techniques of other civilizations to build tunnels in their territories",
      "zh": "曾经控制欧洲、北非和小亚细亚地区的罗马人，采用了其他文明的建筑技术在他们的领土上建造隧道。"
    },
    {
      "id": "s3",
      "en": "The Persians, who lived in present-day Iran, were one of the first civilizations to build tunnels that provided a relia ble supply of water to human settlements in dry areas.",
      "zh": "波斯人，生活在现今的伊朗，是最早建造隧道以向干旱地区的人类聚落提供可靠水源的文明之一。"
    },
    {
      "id": "s4",
      "en": "In the early first millennium BCE, they introduce d the qanat method of tunnel construction, which consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route, and then digging vertical shafts down into the ground at regular intervals.",
      "zh": "公元前一千年初期，他们引入了坎井（qanat）隧道建设方法，即沿山丘直线放置标杆，确保隧道保持路线，然后定期挖掘垂直竖井。"
    },
    {
      "id": "s5",
      "en": "Underground, workers removed the earth from between the ends of the shafts, creating a tunnel.",
      "zh": "地下工人从竖井两端之间挖出土壤，形成隧道。"
    },
    {
      "id": "s6",
      "en": "The excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work.",
      "zh": "挖出的土壤通过竖井被运送至地面，竖井在施工期间还起到了通风作用。"
    },
    {
      "id": "s7",
      "en": "Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use.",
      "zh": "隧道建成后，水从山坡顶流向运河，为人类使用提供水源。"
    },
    {
      "id": "s8",
      "en": "Remarkably, some qanats built by the Persians 2,700 years ago are still in use today.",
      "zh": "令人惊讶的是，一些由波斯人在2700年前建造的坎井至今仍在使用。"
    },
    {
      "id": "s9",
      "en": "They later passed on their knowledge to the Romans, who also used the qanat method to construct water-supply tunnels for agriculture.",
      "zh": "他们后来将知识传授给罗马人，罗马人也采用坎井方法建造农业用供水隧道。"
    },
    {
      "id": "s10",
      "en": "Roman qanat tunnels were constructed with vertical shafts dug at intervals of between 30 and 60 meters.",
      "zh": "罗马的坎井隧道沿线每隔30至60米开挖一个垂直竖井。"
    },
    {
      "id": "s11",
      "en": "The shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid.",
      "zh": "竖井配有手把和脚踏，以帮助人们上下，并盖有木质或石质盖板。"
    },
    {
      "id": "s12",
      "en": "To ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft.",
      "zh": "为确保竖井垂直，罗马人从竖井顶部横杆挂下铅垂线，确保线末端的重物悬挂在竖井中央。"
    },
    {
      "id": "s13",
      "en": "Plumb lines were also used to measure the depth of the shaft and to determine the slope of the tunnel.",
      "zh": "铅垂线还用于测量竖井深度和确定隧道坡度。"
    },
    {
      "id": "s14",
      "en": "The 5.6-kilometer-long Claudius tunnel, built in 41 CE to drain the Fucine Lake in central Italy, had shafts that were up to 122 meters deep, took 11 years to build and involved approximately 30,000 workers.",
      "zh": "克劳狄隧道长5.6公里，于公元41年建成，用于排干意大利中部的福奇内湖，竖井深达122米，耗时11年，约有3万名工人参与。"
    },
    {
      "id": "s15",
      "en": "By the 6th century BCE, a second method of tunnel construction appeared called the counter-excavation method, in which the tunnel was constructed from both ends.",
      "zh": "到公元前6世纪，出现了第二种隧道建造方法——对掘法，即从隧道两端同时开挖。"
    },
    {
      "id": "s16",
      "en": "It was used to cut through high mountains when the qanat method was not a practical alternative.",
      "zh": "当坎井法不适用时，对掘法用于穿越高山。"
    },
    {
      "id": "s17",
      "en": "This method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain.",
      "zh": "此方法需要更周密的计划以及测量、数学和几何学的高级知识，以确保隧道两端能准确在山体中心会合。"
    },
    {
      "id": "s18",
      "en": "Adjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path.",
      "zh": "每当遇到地质问题或隧道偏离预定路线时，施工人员需调整隧道方向。"
    },
    {
      "id": "s19",
      "en": "They constantly checked the tunnel's advancing direction, for example, by looking back at the light that penetrated through the tunnel mouth, and made corrections whenever necessary.",
      "zh": "他们不断检查隧道前进方向，例如通过观察从隧道口透入的光线，必要时进行修正。"
    },
    {
      "id": "s20",
      "en": "Large deviations could happen, and they could result in one end of the tunnel not being usable.",
      "zh": "较大偏差可能发生，导致隧道一端无法使用。"
    },
    {
      "id": "s21",
      "en": "An inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Algeria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.",
      "zh": "在现代阿尔及利亚的萨尔代水渠系统中，一段长428米的罗马隧道侧面刻有铭文，描述了两组施工队在山中未能对接的情况，以及后来修建横向连接通道纠正了这一初期错误。"
    },
    {
      "id": "s22",
      "en": "The Romans dug tunnels for their roads using the counter-excavation method, whenever they encountered obstacles such as hills or mountains that were too high for roads to pass over.",
      "zh": "每当道路遇到如山丘或高山等障碍，罗马人便采用对掘法挖掘隧道以通行。"
    },
    {
      "id": "s23",
      "en": "An example is the 37-meter-long, 6-meter-high, Furlo Pass Tunnel built in Italy in 69-79 CE.",
      "zh": "例如意大利建于公元69至79年的弗尔洛隘口隧道，长37米，高6米。"
    },
    {
      "id": "s24",
      "en": "Remarkably, a modern road still uses this tunnel today.",
      "zh": "令人惊讶的是，现代道路至今仍沿用此隧道。"
    },
    {
      "id": "s25",
      "en": "Tunnels were also built for mineral extraction.",
      "zh": "隧道还用于矿产开采。"
    },
    {
      "id": "s26",
      "en": "Miners would locate a mineral vein and then pursue it with shafts and tunnels underground.",
      "zh": "矿工会找到矿脉，并通过竖井和地下隧道开采。"
    },
    {
      "id": "s27",
      "en": "Traces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales.",
      "zh": "在威尔士的多劳科锡金矿遗址仍能发现曾用于采金的隧道痕迹。"
    },
    {
      "id": "s28",
      "en": "When the sole purpose of a tunnel was mineral extraction, construction required less planning, as the tunnel route was determined by the mineral vein.",
      "zh": "当隧道唯一目的是矿产开采时，建设计划较少，因为隧道路线由矿脉决定。"
    },
    {
      "id": "s29",
      "en": "Roman tunnel projects were carefully planned and carried out.",
      "zh": "罗马隧道项目规划周密，执行严谨。"
    },
    {
      "id": "s30",
      "en": "The length of time it took to construct a tunnel depended on the method being used and the type of rock being excavated.",
      "zh": "隧道建设所需时间取决于所用方法和所挖掘的岩石类型。"
    },
    {
      "id": "s31",
      "en": "The qanat construction method was usually faster than the counter-excavation method as it was more straightforward.",
      "zh": "坎井法通常比对掘法更快，因为它较为直接。"
    },
    {
      "id": "s32",
      "en": "This was because the mountain could be excavated not only from the tunnel mouths but also from shafts.",
      "zh": "这是因为山体不仅可从隧道口，还能从竖井处挖掘。"
    },
    {
      "id": "s33",
      "en": "The type of rock could also influence construction times.",
      "zh": "岩石类型也会影响施工时间。"
    },
    {
      "id": "s34",
      "en": "When the rock was hard, the Romans employed a technique called fire quenching which consisted of heating the rock with fire, and then suddenly cooling it with cold water so that it would crack.",
      "zh": "当岩石坚硬时，罗马人使用一种称为火熄法的技术，即先用火加热岩石，再用冷水急冷使其开裂。"
    },
    {
      "id": "s35",
      "en": "Progress through hard rock could be very slow, and it was not uncommon for tunnels to take years, if not decades, to be built.",
      "zh": "穿透坚硬岩石的进度可能非常缓慢，隧道建设花费数年甚至数十年并不罕见。"
    },
    {
      "id": "s36",
      "en": "Construction marks left on a Roman tunnel in Bologna show that the rate of advance through solid rock was 30 centimeters per day.",
      "zh": "意大利博洛尼亚一条罗马隧道上的施工痕迹显示，穿越坚硬岩石的进度为每天30厘米。"
    },
    {
      "id": "s37",
      "en": "In contrast, the rate of advance of the Claudius tunnel can be calculated at 1.4 meters per day.",
      "zh": "相比之下，克劳狄隧道的推进速度可计算为每天1.4米。"
    },
    {
      "id": "s38",
      "en": "Most tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect.",
      "zh": "大多数隧道刻有订建者的姓名，有时还有建筑师的名字。"
    },
    {
      "id": "s39",
      "en": "For example, the 1.4-kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Seleuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81 CE.",
      "zh": "例如，位于土耳其、长1.4公里的切夫利克隧道为分流威胁古城塞琉西亚皮里亚港口的洪水而建，隧道入口处仍可见铭文，显示该隧道始建于公元69年，完工于81年。"
    }
  ]
---
<p style="text-align:center;"><span style="font-size:22px;"><strong>Roman tunnels</strong></span></p><p style="text-align:center;"><i>The Romans, who once controlled areas of Europe, North Africa and Asia Minor, adopted the construction techniques of other civilizations to build tunnels in their territories</i></p><p style="text-align:center;">&nbsp;</p><p style="text-align:left;">The Persians, who lived in present-day Iran, were one of the first civilizations to build tunnels that provided a relia<span style="font-size:16px;">ble supply of water to human settlements in dry areas. In the early first millennium BCE, they introduce</span>d the <i>qanat</i> method of tunnel construction, which consisted of placing posts over a hill in a straight line, to ensure that the tunnel kept to its route, and then digging vertical shafts down into the ground at regular intervals. Underground, workers removed the earth from between the ends of the shafts, creating a tunnel. The excavated soil was taken up to the surface using the shafts, which also provided ventilation during the work. Once the tunnel was completed, it allowed water to flow from the top of a hillside down towards a canal, which supplied water for human use. Remarkably, some <i>qanats</i> built by the Persians 2,700 years ago are still in use today.</p><p style="text-align:left;">&nbsp;</p><p style="text-align:left;">They later passed on their knowledge to the Romans, who also used the <i>qanat</i> method to construct water-supply tunnels for agriculture. Roman <i>qanat</i> tunnels were constructed with vertical shafts dug at intervals of between 30 and 60 meters. The shafts were equipped with handholds and footholds to help those climbing in and out of them and were covered with a wooden or stone lid. To ensure that the shafts were vertical, Romans hung a plumb line from a rod placed across the top of each shaft and made sure that the weight at the end of it hung in the center of the shaft. Plumb lines were also used to measure the depth of the shaft and to determine the slope of the tunnel. The 5.6-kilometer-long Claudius tunnel, built in 41 CE to drain the Fucine Lake in central Italy, had shafts that were up to 122 meters deep, took 11 years to build and involved approximately 30,000 workers.</p><p style="text-align:left;">&nbsp;</p><p style="text-align:left;">By the 6th century BCE, a second method of tunnel construction appeared called the <i>counter-excavation</i> method, in which the tunnel was constructed from both ends. It was used to cut through high mountains when the <i>qanat</i> method was not a practical alternative. This method required greater planning and advanced knowledge of surveying, mathematics and geometry as both ends of a tunnel had to meet correctly at the center of the mountain. Adjustments to the direction of the tunnel also had to be made whenever builders encountered geological problems or when it deviated from its set path. They constantly checked the tunnel's advancing direction, for example, by looking back at the light that penetrated through the tunnel mouth, and made corrections whenever necessary. Large deviations could happen, and they could result in one end of the tunnel not being usable. An inscription written on the side of a 428-meter tunnel, built by the Romans as part of the Saldae aqueduct system in modern-day Algeria, describes how the two teams of builders missed each other in the mountain and how the later construction of a lateral link between both corridors corrected the initial error.</p><p style="text-align:left;">&nbsp;</p><p style="text-align:left;">The Romans dug tunnels for their roads using the <i>counter-excavation</i> method, whenever they encountered obstacles such as hills or mountains that were too high for roads to pass over. An example is the 37-meter-long, 6-meter-high, Furlo Pass Tunnel built in Italy in 69-79 CE. Remarkably, a modern road still uses this tunnel today. Tunnels were also built for mineral extraction. Miners would locate a mineral vein and then pursue it with shafts and tunnels underground. Traces of such tunnels used to mine gold can still be found at the Dolaucothi mines in Wales. When the sole purpose of a tunnel was mineral extraction, construction required less planning, as the tunnel route was determined by the mineral vein.</p><p style="text-align:left;">&nbsp;</p><p style="text-align:left;">Roman tunnel projects were carefully planned and carried out. The length of time it took to construct a tunnel depended on the method being used and the type of rock being excavated. The <i>qanat</i> construction method was usually faster than the <i>counter-excavation</i> method as it was more straightforward. This was because the mountain could be excavated not only from the tunnel mouths but also from shafts. The type of rock could also influence construction times. When the rock was hard, the Romans employed a technique called fire quenching which consisted of heating the rock with fire, and then suddenly cooling it with cold water so that it would crack. Progress through hard rock could be very slow, and it was not uncommon for tunnels to take years, if not decades, to be built. Construction marks left on a Roman tunnel in Bologna show that the rate of advance through solid rock was 30 centimeters per day. In contrast, the rate of advance of the Claudius tunnel can be calculated at 1.4 meters per day. Most tunnels had inscriptions showing the names of patrons who ordered construction and sometimes the name of the architect. For example, the 1.4-kilometer Cevlik tunnel in Turkey, built to divert the floodwater threatening the harbor of the ancient city of Seleuceia Pieria, had inscriptions on the entrance, still visible today, that also indicate that the tunnel was started in 69 CE and was completed in 81 CE.</p>
